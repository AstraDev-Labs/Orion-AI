...
**`orion self-update`** subcommand. Detects how Orion was
installed (pip, uv tool, editable git checkout) by inspecting
`orion.__file__`, then runs the right upgrade command. Supports
...
(`OPENORION_NO_UPDATER=1`) documented in `docs/desktop-auto-update.md`.

**Analytics env-var opt-out** (`DO_NOT_TRACK`, `OPENORION_NO_ANALYTICS`).
...
upgrade command. Honors `ORION_NO_UPDATE_CHECK=1` and `CI=true` to
...
- **Editable-git users running `orion self-update`** get the
  detected `git pull && uv sync` command pointed at their actual
  checkout, not `~/Orion`. If you'd come to rely on the
...
Companion blog post:
[From Minions to Orion: A Retrospective on Two Years in Local AI](https://hazyresearch.stanford.edu/blog/2026-05-19-minions-to-orion-retrospective).
...
`OrionSystem` in `src/orion/system.py`, driven by a TOML config.
...
`orion init --preset <name>` bundle an agent with a hardware-appropriate
...
(`src/orion/learning/routing/complexity.py`). Produces a 0.0–1.0
...
**LLM-guided spec search** (`src/orion/learning/spec_search/`).
...
preset and a runnable tutorial at `examples/orion/spec_search_quickstart.py`.

**Six hybrid coordination paradigms** in `src/orion/agents/hybrid/`.
...
A runner CLI (`python -m orion.agents.hybrid.runner --cell <name>`)
...
- **agentskills.io spec adoption** — canonical `SKILL.md` format with YAML frontmatter following the [agentskills.io](https://agentskills.io/specification) open standard.
  ...
  - Sourced subdirectory layout (`~/.orion/skills/<source>/<name>/`)
...
- **Skills learning loop** — trace tagging, pattern discovery, DSPy/GEPA optimization.
  ...
  - `SkillOverlay` — sidecar storage at `~/.orion/learning/skills/<name>/optimized.toml`
...
- **Skills benchmark harness** — 4-condition PinchBench evaluation.
  ...
  - `OrionAgentBackend` accepts `skills_enabled` and `overlay_dir` kwargs
  - Conditions: `no_skills`, `skills_on`, `skills_optimized_dspy`, `skills_optimized_gepa`

- **CLI commands:**
  - `orion skill list` / `info` / `run` / `install` / `sync` / `sources` / `update` / `remove` / `search`
  - `orion skill discover` — mine traces for recurring tool patterns
  - `orion skill show-overlay` — inspect optimization output
  - `orion optimize skills` — run DSPy/GEPA per-skill optimization
  - `orion bench skills` — run the PinchBench skills benchmark
...
- `examples/orion/spec_search_quickstart.py` — runnable end-to-end
...
- `src/orion/agents/hybrid/registry/*.toml` — 35-cell registry of
...
- **`learning/distillation/` is now `learning/spec_search/`.** The
  ...
  (`from orion.learning.distillation.*` →
  `from orion.learning.spec_search.*`). The `orion distillation`
...
- **Engine `generate_full` return shape extended.**
  `OrionAgentBackend.generate_full` and `OrionDirectBackend.generate_full`
...
- **OpenClaw category/name queries** — `orion skill install openclaw:owner/slug` now correctly splits into category + name match.
