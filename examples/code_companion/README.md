...
A set of developer-focused scripts that use Orion tool-using agents
to automate common coding tasks: code review, debugging, and test generation.

## What This Demonstrates

Each script wires up a `Orion` instance with the `native_react` (ReAct) agent
...
1. **Install Orion** (from the repo root):
...
```bash
python examples/code_companion/test_gen.py --module src/orion/tools/calculator.py
```
...
python examples/code_companion/test_gen.py \
    --module src/orion/tools/calculator.py \
    --framework unittest \
...
`memory_store`, `memory_search`, and more. Run `uv run orion eval list` or
inspect `src/orion/tools/` for the full registry.
