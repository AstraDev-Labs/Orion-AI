...
This tutorial demonstrates how to use Orion to run **autonomous scheduled agents** that perform recurring personal tasks on a cron-like schedule.

## What This Demonstrates

- Using the `Orion` SDK with different agent types (`orchestrator`, `native_react`) and tool sets
- Configuring recurring schedules via TOML and the `orion scheduler` CLI
...
Register each script as a recurring task with `orion scheduler create`:
...
uv run python examples/scheduled_ops/daily_digest.py | orion channel send slack
...
See `orion channel list` for all available channels.
...
- **Model selection**: Use `--model` to target a specific model, or let Orion auto-select from what is available.
