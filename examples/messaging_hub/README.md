...
- **Channel integration** — connecting Orion to messaging platforms (Slack, WhatsApp, etc.)
...
- Python 3.10+
- Orion installed: `uv sync --extra dev`
...
2. Configure the WhatsApp Baileys bridge (see the Orion channel docs).
...
Orion supports many channels — LINE, Viber, Mastodon, Rocket.Chat, and
...
Refer to `configs/orion/config.toml` for the full list of channel and agent
options.
...
this via the Orion scheduler:

```bash
orion scheduler create "Daily inbox summary" --type cron --value "0 17 * * *"
```

Or use the operator recipe pattern to run a persistent triage agent on a
schedule. See `src/orion/recipes/data/operators/` for examples.
