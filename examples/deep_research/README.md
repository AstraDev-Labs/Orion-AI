...
using Orion. The assistant uses an orchestrator agent loop with web
...
- Orion installed (`uv sync --extra dev` from the repo root)
...
recipe that can be loaded with `load_recipe()` or passed to the `orion eval`
runner.
...
User query
  |
  v
Orion SDK  (model + engine selection)
  |
  v
...
- **Add more tools** -- append tool names to the `tools` list in
  `research.toml` or pass them on the command line. See `orion agent info
...
- **Use the recipe programmatically** -- load the TOML with
  `orion.recipes.load_recipe("examples/deep_research/research.toml")` and
...
- [Recipes](../../src/orion/recipes/) -- composable TOML configs that
