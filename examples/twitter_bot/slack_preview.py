...
TOPICS = [
    # Narrative — opinionated takes that happen to be about Orion
...
FACTS = [
    ...
    "In nearly all personal AI projects today, the local component is a thin orchestration layer, while the brain lives in someone else data center. Your most personal data routes through cloud APIs, with their latency, their cost, and their terms of service. We built Orion to fix this.",
    "Orion is structured around five composable primitives: Intelligence, Engine, Agents, Tools and Memory, and Learning. Each primitive can be benchmarked, substituted, and optimized independently.",
    "Orion includes hardware-agnostic telemetry that profiles energy consumption across NVIDIA GPUs, AMD GPUs, and Apple Silicon. Energy and dollar cost are first-class design constraints alongside accuracy.",
    ...
    "Orion is open source under Apache 2.0, built at Stanford at Hazy Research and the Scaling Intelligence Lab at SAIL. Because the tools for studying and building local-first AI should be available to everyone.",
    "Orion supports 27 channel integrations including Slack, Discord, Telegram, WhatsApp. Adding a new channel is one file implementing BaseChannel with connect, send, and disconnect.",
    "Orion supports multiple inference engines: Ollama, vLLM, SGLang, llama.cpp. orion init picks the right one for your hardware.",
    "Install Orion by running git clone https://github.com/open-orion/Orion.git then cd Orion then uv sync. Use orion init to auto-detect hardware and configure the engine.",
    "Orion memory and RAG supports four backends: SQLite FTS5 for keyword search, FAISS for vector similarity, ColBERT for token-level matching, and BM25 for probabilistic retrieval.",
    "Orion ships with nine example projects: deep_research, code_companion, messaging_hub, scheduled_ops, browser_assistant, security_scanner, daily_digest, doc_qa, and multi_model_router.",
]
...
def main() -> None:
    ...
    parent_ts = slack_send(
        "*Orion Twitter Bot — Overnight Preview*\n"
    ...
    while True:
        ...
        tool_log.clear()
        r = j.ask_full(
            "You are @OrionAI — a researcher/dev who believes local-first "
            ...
            'server. we built orion to fix that"\n'
            ...
            "Link: https://github.com/open-orion/Orion",
            agent="orchestrator",
