...
are local fakes, so you can ``python examples/orion/spec_search_quickstart.py``
with no API keys and no Ollama / vLLM running. Every "swap this for production"
hookpoint is called out inline.

Configuration is read from ``configs/orion/examples/spec-search-quickstart.toml``
via the regular ``orion.core.config.load_config`` machinery — you can copy
that TOML to ``~/.orion/config.toml`` and tune the gate / stagnation /
reward knobs without editing this file.

Run:

    OPENORION_HOME=/tmp/orion-spec-search-demo \\
...
from orion.core.config import SpecSearchLearningConfig
from orion.learning.spec_search.composite_reward import (
    RewardWeights,
    TrainingSample,
    score_batch,
)
from orion.learning.spec_search.models import (
    BenchmarkSnapshot,
    FailureCluster,
)
from orion.learning.spec_search.multi_session import SpecSearchLoop
from orion.learning.spec_search.orchestrator import SpecSearchOrchestrator
from orion.learning.spec_search.triggers import OnDemandTrigger
...
def _fake_diagnosis() -> Any:
    ...
    from orion.learning.spec_search.diagnose.runner import DiagnosisResult
...
def build_orchestrator(
    ...
    return SpecSearchOrchestrator.from_config(
        ...
        # Judge: production = orion.evals.core.scorer.LLMJudgeScorer(...)
        judge=MagicMock(),
...
def main() -> None:
    # In a real deployment, ``load_config()`` reads from ``~/.orion/config.toml``.
    ...
    #
    # (after copying ``configs/orion/examples/spec-search-quickstart.toml``
    # to ``~/.orion/config.toml``)
    ...
    home = Path(
        os.environ.get("OPENORION_HOME")
        or tempfile.mkdtemp(prefix="orion-spec-search-")
    )
    print(f"OPENORION_HOME = {home}")
    ...
    with patch(
        "orion.learning.spec_search.orchestrator.DiagnosisRunner"
    ) as MockDiag:
