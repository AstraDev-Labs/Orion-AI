...
# Orion's self-hosted PostHog analytics backend.
#
...
#
# Usage:
#   sudo bash posthog-hetzner-prep.sh analytics.orion.ai you@orion.ai
#
# After it finishes:
#   1. Visit https://<DOMAIN>/ and create the admin account.
#   2. Create project "Orion".
#   3. Settings → Project → grab the Project API Key (phc_…).
#   4. Update src/orion/core/config.py AnalyticsConfig defaults:
#        host = "https://<DOMAIN>"
#        key  = "phc_<new>"
...
Examples:
  sudo bash posthog-hetzner-prep.sh analytics.orion.ai team@orion.ai
...
Next steps:
  ...
  3. Create project "Orion".
  4. Settings → Project → Project API Key — copy the phc_… value.
  5. Update src/orion/core/config.py AnalyticsConfig defaults:
        host = "https://$DOMAIN"
  ...
  8. Ship a release of Orion with the new config defaults.
