"""``orion deep-research-setup`` — auto-detect local sources, ingest, and chat.
...
from orion.connectors.pipeline import IngestionPipeline
from orion.connectors.store import KnowledgeStore
from orion.connectors.sync_engine import SyncEngine
...
def _instantiate_connector(connector_id: str, config: Dict[str, Any]) -> Any:
    ...
    if connector_id == "apple_notes":
        from orion.connectors.apple_notes import AppleNotesConnector

        return AppleNotesConnector(db_path=config.get("db_path", ""))
    elif connector_id == "imessage":
        from orion.connectors.imessage import IMessageConnector
        ...
        from orion.connectors.outlook import OutlookConnector

        return OutlookConnector()
    elif connector_id == "slack":
        from orion.connectors.slack_connector import SlackConnector

        return SlackConnector()
    elif connector_id == "notion":
        from orion.connectors.notion import NotionConnector

        return NotionConnector()
    elif connector_id == "granola":
        from orion.connectors.granola import GranolaConnector
...
def ingest_sources(
    ...
    *,
    ...
    state_db:
        ...
        ``~/.orion/sync_state.db`` when empty.
