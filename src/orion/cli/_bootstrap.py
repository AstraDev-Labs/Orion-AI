...
Used by both ``install.sh`` (via ``orion _bootstrap --write-config``)
and ``orion init`` (so there is a single source of truth for the
...
import orion
from orion.core import config as _cfg
...
def write_initial_config(
    *,
    ...
    Called by both ``install.sh`` (via ``orion _bootstrap --write-config``)
    and ``orion init`` so the TOML format has one definition.
    """
    ...
    base_toml = (
        f"# Orion configuration\n"
        f"{hardware_line}{gpu_comment}\n"
        f"# Full reference config: orion init --full\n"
        f"\n"
        f"{provenance}\n"
        f"\n"
        f"[engine]\n"
...
# ---------------------------------------------------------------------------
# CLI command — invoked by install.sh, hidden from `orion --help`
# ---------------------------------------------------------------------------
