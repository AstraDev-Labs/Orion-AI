"""``orion doctor`` — run diagnostic checks on the Orion installation."""
...
from orion.core.config import DEFAULT_CONFIG_PATH, load_config
...
def _check_config_exists() -> CheckResult:
    ...
    return CheckResult(
        ...
        details="Run `orion init` to generate a config file.",
    )
...
def _check_optional_deps() -> List[CheckResult]:
    ...
    results: List[CheckResult] = []
    optional_packages = [
        ("fastapi", "orion[server]", "REST API server"),
        ("torch", "pip install torch", "SFT/GRPO training"),
        ("pynvml", "orion[gpu-metrics]", "NVIDIA energy monitoring"),
        ("amdsmi", "orion[energy-amd]", "AMD energy monitoring"),
        ("colbert", "orion[memory-colbert]", "ColBERT memory backend"),
        ("zeus", "orion[energy-apple]", "Apple Silicon energy monitoring"),
    ]
...
def doctor(as_json: bool) -> None:
    """Run diagnostic checks on your Orion installation."""
    ...
    console = Console()
    console.print()
    console.print("[bold]Orion Doctor[/bold]")
    console.print()
    ...
    from orion.cli._bg_state import get_status
    ...
    if bg.rust_extension == "ready":
        ...
        console.print(
            "    retry: ~/.orion/.scripts/install-rust.sh && "
            "~/.orion/.scripts/build-extension.sh"
        )
        bg_failed = True
    else:
        console.print(
    ...
    for model_id, state in bg.models.items():
        if state == "ready":
            ...
            console.print(f"    retry: ~/.orion/.scripts/pull-model.sh {model_id}")
            bg_failed = True
        else:
