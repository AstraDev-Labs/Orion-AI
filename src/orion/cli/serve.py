"""``orion serve`` — OpenAI-compatible API server."""
...
def serve(
        ...
        try:
            ...
            if AgentRegistry.contains(agent_key):
                ...
                if getattr(agent_cls, "accepts_tools", False):
                    import orion.tools  # noqa: F401  # trigger registration
    ...
    api_key = _os.environ.get("OPENORION_API_KEY", "")
    if not api_key:
        try:
            import tomllib

            _cfg_path = str(
                __import__("pathlib").Path.home() / ".orion" / "config.toml"
            )
    ...
    from orion.server.auth_middleware import check_bind_safety

    check_bind_safety(bind_host, api_key=api_key)

    # Log credential status at startup
    from orion.core.credentials import TOOL_CREDENTIALS, get_credential_status
    ...
    console.print(
        f"[green]Starting Orion API server[/green]\n"
