"""``orion agents`` — persistent agent lifecycle management."""
...
def list_agents() -> None:
    ...
    try:
        ...
        if not agents:
            console.print(
                "[dim]No agents found. Create one with: orion agents create[/dim]"
            )
            return
...
def _get_system():
    """Build a OrionSystem for CLI commands that need scheduler/executor."""
...
def _get_scheduler_and_executor(system=None):
    """Get scheduler + executor from a OrionSystem instance."""
    if system is None:
        system = _get_system()
...
def launch():
    ...
    from orion.agents.manager import AgentManager as _AM
