"""``orion channels`` — manage messaging channels for the agent."""
...
def channels_status() -> None:
    ...
    from orion.channels.imessage_daemon import is_running
    ...
    if is_running():
        ...
        table.add_row(
            "iMessage",
            "[dim]stopped[/dim]",
            "orion channels imessage-start <contact>",
        )
    ...
    if background:
        ...
        proc = subprocess.Popen(
            [
                sys.executable,
                "-m",
                "orion.channels.imessage_daemon",
                "--chat",
                chat_identifier,
            ],
        ...
        from orion.connectors.retriever import (
            TwoStageRetriever,
        )
        from orion.connectors.store import KnowledgeStore
        from orion.engine.ollama import OllamaEngine
        from orion.tools.knowledge_search import (
            KnowledgeSearchTool,
        )
        from orion.tools.knowledge_sql import (
            KnowledgeSQLTool,
        )
...
def imessage_stop() -> None:
    ...
    from orion.channels.imessage_daemon import stop_daemon
