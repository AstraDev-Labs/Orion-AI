"""``orion init`` — detect hardware, generate config, write to disk."""
...
from orion.cli._bootstrap import detect_cloud_keys
from orion.cli.model import find_model_spec, hf_download, ollama_pull
...
# Engines supported by ``orion init --engine``.
_SUPPORTED_ENGINES = [
    "ollama",
    "vllm",
    "sglang",
...
def _next_steps_text(engine: str, model: str = "") -> str:
    ...
    steps: dict[str, str] = {
        "ollama": (
            ...
            '     orion ask "Hello"\n'
            "\n"
            "  Run `orion doctor` to verify your setup."
        ),
        "vllm": (
            "Next steps:\n"
            "\n"
...
def _quick_privacy_check(console: Console) -> None:
    ...
    console.print()
    console.print("  Run [cyan]orion scan[/cyan] for a full environment audit.")
...
@click.option(
    ...
    default=False,
    hidden=True,
    help="Run init non-interactively; called by the bare-orion first-run guard.",
)
def init(
    force: bool,
    ...
    from_bare_orion: bool = False,
) -> None:
    """Detect hardware and generate ~/.orion/config.toml."""
    console = Console()
    ...
    if preset:
        examples_dir = (
            Path(__file__).resolve().parents[2] / "configs" / "orion" / "examples"
        )
        ...
        console.print(
            ...
            "[bold]orion doctor[/bold] to verify."
        )
        return
    ...
    if engine is None and config is None:
        ...
        # Bare-orion cold path: use the recommended engine non-interactively.
        if from_bare_orion:
            engine = recommended
        else:
            console.print()
...
"""
        ...
        console.print(
            ...
            "Google services, then [bold]orion digest --fresh[/bold]."
        )
    ...
    if not soul_path.exists():
        soul_path.write_text(
            "# Agent Persona\n\nYou are Orion, a helpful personal AI assistant.\n"
        )
    ...
    if not model:
        ...
        if not no_download and not from_bare_orion and spec:
            ...
            if click.confirm(prompt, default=True):
                ...
                console.print(
                    ...
                    f"    [bold]orion model pull {model}[/bold]"
                )
