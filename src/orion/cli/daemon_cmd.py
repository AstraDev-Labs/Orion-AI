"""``orion start|stop|restart|status`` — daemon management commands."""
...
@click.group()
def daemon() -> None:
    """Manage the Orion server daemon."""
...
def start(
    ...
    """Start the Orion server as a background daemon."""
    console = Console(stderr=True)

    existing = _read_pid()
    if existing is not None:
        console.print(f"[yellow]Server already running (PID {existing}).[/yellow]")
        console.print("Use 'orion stop' to stop it first, or 'orion restart'.")
        sys.exit(1)
    ...
    # Build command to run orion serve
    cmd = [sys.executable, "-m", "orion.cli", "serve"]
    if host:
    ...
    console.print(
        f"[green]Orion server started[/green] (PID {proc.pid})\n"
...
@daemon.command()
def stop() -> None:
    """Stop the running Orion server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
...
def restart(ctx: click.Context) -> None:
    """Restart the Orion server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is not None:
...
@daemon.command()
def status() -> None:
    """Show status of the Orion server daemon."""
    console = Console(stderr=True)
    pid = _read_pid()
    if pid is None:
