...
def _get_discovered_dir() -> Path:
    ...
    return Path("~/.orion/skills/discovered/").expanduser()


def _get_overlay_dir() -> Path:
    """Return the directory where optimization overlays are stored."""
    return Path("~/.orion/learning/skills/").expanduser()
...
def _get_skill_paths() -> List[Path]:
    ...
    user_dir = Path("~/.orion/skills/").expanduser()
...
def _get_resolver(source: str, url: str = ""):
    ...
    if source == "hermes":
        from orion.skills.sources.hermes import HermesResolver

        return HermesResolver()
    if source == "openclaw":
        from orion.skills.sources.openclaw import OpenClawResolver
    ...
    if source == "github":
        ...
        cache = _Path(
            "~/.orion/skill-cache/github/" + url.rstrip("/").rsplit("/", 1)[-1]
        ).expanduser()
...
def install(query: str, with_scripts: bool, force: bool, url: str):
    ...
    Example: ``orion skill install hermes:apple-notes``
    """
    console = Console()
    ...
    from orion.skills.importer import SkillImporter
    from orion.skills.parser import SkillParser
    from orion.skills.tool_translator import ToolTranslator
...
def remove(skill_name: str, yes: bool):
    ...
    Searches ``~/.orion/skills/`` and ``./skills`` for a directory whose
...
def search(query: str, source: str):
    ...
    console.print(
        ...
        f"Install with: orion skill install <source>:<name>[/dim]"
    )
...
def discover(min_frequency: int, min_outcome: float, dry_run: bool) -> None:
    ...
    discovered skill manifests under ~/.orion/skills/discovered/."""
    console = Console()
    ...
    if dry_run:
        ...
        tmp = Path(tempfile.mkdtemp(prefix="orion-discover-dryrun-"))
        try:
