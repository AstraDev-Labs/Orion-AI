"""``orion optimize`` — LLM-driven configuration optimization CLI."""
...
def optimize_run(
    ...
    """Run an optimization loop over Orion configuration."""
    ...
    if config_path is not None:
        try:
            from orion.learning.optimize.config import load_optimize_config
        except ImportError:
    ...
    if data is not None:
        try:
            from orion.learning.optimize.config import load_benchmark_specs
    ...
    try:
        ...
        from orion.learning.optimize.config import load_objectives
        ...
        from orion.learning.optimize.search_space import (
            DEFAULT_SEARCH_SPACE,
            build_search_space,
        )
        from orion.learning.optimize.store import OptimizationStore
        from orion.learning.optimize.trial_runner import (
        ...
        try:
            from orion.evals.cli import _build_judge_backend
...
def skills(policy: str, min_traces: int, dry_run: bool) -> None:
    ...
    from orion.learning.agents.skill_optimizer import SkillOptimizer
