"""``orion ask`` — send a query to the assistant."""
...
from orion.cli._tool_names import resolve_tool_names
from orion.cli.hints import hint_no_engine
...
def _run_research(
    *,
    ...
    from orion.agents.research_loop import DEFAULT_PLANNER_MODEL, ResearchAgent
    from orion.connectors.embeddings import OllamaEmbedder
    from orion.connectors.hybrid_search import HybridSearch
    ...
    # (so ``orion ask --research "..." > out.md`` still gives a clean
...
def _get_memory_backend(config):
    ...
    """
    try:
        import orion.connectors.store  # noqa: F401
        ...
        try:
            ...
            if key != "knowledge" and MemoryRegistry.contains("knowledge"):
                logger.debug(
                    "Memory backend %r unavailable (%s); using knowledge",
                    key,
                    exc,
                )
                backend = MemoryRegistry.create("knowledge")
            else:
                raise
...
def _run_agent(
    ...
    if "prompt_builder" in _inspect.signature(agent_cls.__init__).parameters:
        from orion.prompt.builder import SystemPromptBuilder
...
def ask(
    ...
    # the most common command (`orion ask "..."`).
    ...
    if resolved is None:
        console.print(
            ...
            "  [cyan]orion config set engine.ollama.host http://<remote-ip>:11434[/cyan]\n"
