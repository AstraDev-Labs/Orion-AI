"""``orion host`` — download and serve a model locally with auto backend setup."""
...
def host(
    ...
    \b
      orion host mlx-community/Qwen2.5-7B-4bit --backend mlx
      orion host Qwen/Qwen3-8B --backend vllm
      orion host qwen3:8b --backend ollama
      orion host meta-llama/Llama-3-8B -b sglang
    """
    console = Console()
    ...
    if backend != "ollama":
        ...
        console.print(
            "[dim]Orion will auto-discover it. Press Ctrl+C to stop.[/dim]\n"
        )
