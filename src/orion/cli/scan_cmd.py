"""``orion scan`` — audit your environment for privacy and security risks."""
...
class PrivacyScanner:
    ...
    def check_icloud_sync(self) -> ScanResult:
        """Check whether ~/.orion is inside iCloud Drive sync scope."""
        try:
            config_path = Path("~/.orion").expanduser().resolve()
            icloud_path = Path("~/Library/Mobile Documents/").expanduser().resolve()
            if str(config_path).startswith(str(icloud_path)):
                return ScanResult(
                    name="iCloud Sync",
                    status="warn",
                    message="~/.orion may be synced to iCloud.",
                    platform="darwin",
                )
            ...
            return ScanResult(
                name="iCloud Sync",
                status="ok",
                message="~/.orion is not inside iCloud Drive.",
                platform="darwin",
            )
        except Exception:
            return ScanResult(
    ...
    def run_quick(self) -> list[ScanResult]:
        ...
        if sys.platform == "darwin":
            ...
            # No BitLocker check exists yet. Falling through to check_luks here
            # ran a Linux-only disk-encryption probe on Windows and reported a
            # result labelled platform="linux", which is simply wrong.
...
def _render_results(results: List[ScanResult]) -> None:
    ...
    console = Console()
    console.print()
    console.print("[bold]Orion Security Scan[/bold]")
    console.print()
    ...
    if fail_count:
        console.print(
            ...
            "before storing sensitive data with Orion."
        )
        console.print()
    elif warn_count:
        console.print(
