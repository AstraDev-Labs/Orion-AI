"""High-level Python SDK for Orion."""
...
import orion
from orion.core.config import OrionConfig, load_config
from orion.core.events import EventBus
from orion.core.types import Message, Role
from orion.engine._discovery import get_engine
from orion.system import OrionSystem, SystemBuilder
...
class MemoryHandle:
    ...
    def _get_backend(self) -> Any:
        ...
        if not MemoryRegistry.contains(key):
            ...
            try:
                from orion.tools.storage.sqlite import SQLiteMemory  # noqa: F401
            except ImportError:
                pass
    ...
    def index(
        ...
        *,
        ...
        from orion.tools.storage.chunking import ChunkConfig
        from orion.tools.storage.ingest import ingest_path
...
class Orion:
    ...
    @property
    def config(self) -> OrionConfig:
        """Return the active configuration."""
        return self._config

    @property
    def version(self) -> str:
        """Return the Orion version string."""
        return orion.__version__
    ...
    def _run_agent(
        ...
        *,
        ...
        if tools:
            ...
            from orion.cli.ask import _build_tools
        ...
        # Pass the configured system prompt to agents that support it
        _sys_prompt = self._config.agent.default_system_prompt or None
        if _sys_prompt and hasattr(agent_cls, "__init__"):
            import inspect
            if "system_prompt" in inspect.signature(agent_cls.__init__).parameters:
                agent_kwargs["system_prompt"] = _sys_prompt
        ...
        if agent_name == "morning_digest" and hasattr(self._config, "digest"):
            ...
            from orion.tools.digest_collect import DigestCollectTool
            from orion.tools.text_to_speech import TextToSpeechTool
        ...
        if context and self._config.agent.context_from_memory:
            try:
                from orion.cli.ask import _get_memory_backend
                from orion.tools.storage.context import (
                    ContextConfig,
                    inject_context,
                )
...
__all__ = ["Orion", "OrionSystem", "MemoryHandle", "SystemBuilder"]
