...
Uses OAuth 2.0 tokens stored locally (see :mod:`orion.connectors.oauth`).
...
from orion.connectors._stubs import BaseConnector, Document, SyncStatus
from orion.connectors.google_auth import (
    GoogleAuthError,
)
from orion.connectors.google_auth import (
...
class GmailConnector(BaseConnector):
    ...
    def is_connected(self) -> bool:
        ...
        token), which made `orion connect gmail` short-circuit with
    ...
    def _refresh_token(self) -> str:
        ...
        if not new:
            raise RuntimeError(
                "Gmail token refresh failed — re-run `orion connect gmail`"
            )
        return new
