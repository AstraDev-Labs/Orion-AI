...
def get_client_credentials(
    ...
    Checks credential files in ``~/.orion/connectors/`` and falls
    back to environment variables ``OPENORION_{NAME}_CLIENT_ID`` and
    ``OPENORION_{NAME}_CLIENT_SECRET``.
    """
    ...
    prefix = f"OPENORION_{provider.name.upper()}"
        ...
        def do_GET(self) -> None:  # noqa: N802 — required override name
            ...
            if "code" in params:
                ...
                self.wfile.write(
                    ...
                    b"<p>You can close this tab and return to Orion.</p>"
                    b"</body></html>"
                )
