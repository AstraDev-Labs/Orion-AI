...
_DEFAULT_CONFIG_PATH = DEFAULT_CONFIG_DIR / "connectors" / "obsidian.json"
...
class ObsidianConnector(BaseConnector):
    ...
    def __init__(self, vault_path: str = "") -> None:
        if not vault_path and _DEFAULT_CONFIG_PATH.exists():
            try:
                data = json.loads(_DEFAULT_CONFIG_PATH.read_text(encoding="utf-8"))
                vault_path = data.get("vault_path") or data.get("path") or ""
    ...
    def disconnect(self) -> None:
        ...
        try:
            _DEFAULT_CONFIG_PATH.unlink(missing_ok=True)
        except OSError:
            pass
