...
from orion.skills.dependency import validate_dependencies
from orion.skills.executor import SkillExecutor, SkillResult
from orion.skills.loader import discover_skills
from orion.skills.tool_adapter import SkillTool
from orion.skills.types import SkillManifest
...
class SkillManager:
    ...
    def __init__(
        ...
        *,
        ...
        if overlay_dir is None:
            ...
            # ~/.orion/learning/skills/ if config can't be loaded.
            try:
                ...
                pass
            if overlay_dir is None:
                overlay_dir = Path("~/.orion/learning/skills/").expanduser()
    ...
    def _load_overlays(self) -> None:
        ...
        examples under ``manifest.metadata.orion.few_shot``.

        Bad overlays are silently ignored — they should not break discovery.
        """
        from orion.skills.overlay import SkillOverlayLoader
        ...
        for name, manifest in self._skills.items():
            ...
            if overlay.few_shot:
                ...
                oj = dict(new_metadata.get("orion", {}) or {})
                oj["few_shot"] = list(overlay.few_shot)
                new_metadata["orion"] = oj
    ...
    def get_few_shot_examples(self) -> List[str]:
        ...
        Pulls from ``manifest.metadata.orion.few_shot`` for every
        registered skill.  Returns one formatted string per example.
        """
        examples: List[str] = []
        for name, manifest in self._skills.items():
            oj = manifest.metadata.get("orion", {}) if manifest.metadata else {}
    ...
    def discover_from_traces(
        ...
        *,
        ...
        ``~/.orion/skills/discovered/``).  Returns a list of dicts with
        ...
        if output_dir is None:
            output_dir = Path("~/.orion/skills/discovered/").expanduser()
    ...
    def _serialize_discovered_skill(name: str, skill: Any) -> str:
        ...
        lines.append('author = "orion (auto-discovered)"')
    ...
    def find_installed_paths(
        ...
        """
        if roots is None:
            roots = [Path("~/.orion/skills/").expanduser(), Path("./skills")]
        ...
        for root in roots:
            ...
            for candidate in root.rglob("*"):
                ...
                try:
                    from orion.skills.loader import load_skill_directory
