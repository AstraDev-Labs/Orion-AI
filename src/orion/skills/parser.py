...
2. Tolerant pass — maps non-spec top-level fields to their canonical
   locations under metadata.orion.* via FIELD_MAPPING.
...
# dataclass attribute.  When target_kind is "orion_meta" the value is
# stored under manifest.metadata["orion"][attr].
FIELD_MAPPING: Dict[str, tuple[str, str]] = {
    ...
    "platforms": ("orion_meta", "platforms"),
    "prerequisites": ("orion_meta", "prerequisites"),
}
...
class SkillParser:
    ...
    def _build_manifest(
        ...
        captures unknown fields under metadata.orion.original_frontmatter,
        and merges metadata.orion.* into canonical fields.
        """
        ...
        if not isinstance(raw_metadata, dict):
            raw_metadata = {}
        # Initialize orion namespace
        oj_meta = dict(raw_metadata.get("orion") or {})
        ...
        for key, value in frontmatter.items():
            ...
            if key in FIELD_MAPPING:
                ...
                LOGGER.warning(
                    ...
                    "(value preserved in metadata.orion.original_frontmatter)",
                    key,
                    manifest.name,
                )

        # Merge metadata.orion.* into canonical fields (these override
        # top-level mappings since they are explicit Orion-namespaced).
        for key in (
            "version",
            "author",
            "tags",
        ...
        # Preserve unmapped fields under metadata.orion.original_frontmatter
        if unmapped:
            oj_meta["original_frontmatter"] = unmapped

        # Stash the orion metadata block back
        if oj_meta:
            new_metadata = dict(raw_metadata)
            new_metadata["orion"] = oj_meta
