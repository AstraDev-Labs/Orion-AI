...
User configuration lives at ``~/.orion/config.toml``.  ``load_config()``
...
if TYPE_CHECKING:
    # Only used by type-checkers (mypy/pyright) for the ``OrionConfig.mining``
...
def recommend_model(hw: HardwareInfo, engine: str) -> str:
    ...
    """
    from orion.intelligence.model_catalog import BUILTIN_MODELS
...
@dataclass(slots=True)
class NvidiaEngineConfig:
    """Per-engine config for NVIDIA NIM."""

    host: str = "https://integrate.api.nvidia.com"
    api_prefix: str = "/v1"
    api_key: str = ""
...
class EngineConfig:
    ...
    nvidia: NvidiaEngineConfig = field(default_factory=NvidiaEngineConfig)
    ...
    @property
    def nvidia_host(self) -> str:
        """Deprecated: use ``engine.nvidia.host``."""
        return self.nvidia.host

    @nvidia_host.setter
    def nvidia_host(self, value: str) -> None:
        self.nvidia.host = value
...
class RoutingLearningConfig:
    ...
    default_routing: str = "local"  # "local" or "cloud"
    # Additional weights for multi-armed bandit (latency vs cost vs quality)
...
class ACEOptimizerConfig:
    ...
    Install via ``pip install -e orion[learning-ace]`` once the
    ...
    # most Orion users have credentials for.
    ...
    # Empty string defaults to ``~/.orion/learning/ace/<task>/``.
    save_dir: str = ""
    task_name: str = "orion"
...
class SkillsLearningConfig:
    ...
    overlay_dir: str = "~/.orion/learning/skills/"
...
class AgentConfig:
    ...
    default_system_prompt: str = (
        ...
        "did not ask for. "
        "CONNECTED SERVICES: when the user asks to search, read or write in a named "
        "connected service, use that provider's tool; do not substitute opening its website. "
        "For Notion use notion_search_pages / notion_get_page / notion_create_page; "
        "creation requires a page shared with the integration and its page ID. "
        "If the needed provider action is unavailable or credentials fail, say so clearly. "
        "Use web_search for news and anything current. When the user names a connected "
        "service, use that service rather than another note store or its website. Use "
        "obsidian_write_note / obsidian_search_notes only when the user asks for Obsidian "
        "or does not specify a service. "
...
class AnalyticsConfig:
    ...
    usage events sent to the Orion team's PostHog instance to
...
class WhatsAppBaileysChannelConfig:
    ...
    auth_dir: str = ""  # Defaults to ~/.orion/whatsapp_auth
...
class SchedulerConfig:
    ...
    db_path: str = ""  # Defaults to ~/.orion/scheduler.db
...
class OperatorsConfig:
    ...
    enabled: bool = False
    manifests_dir: str = "~/.orion/operators"
...
class SpeechConfig:
    ...
    device: str = "cpu"  # "auto", "cpu", "cuda" — cpu avoids missing CUDA DLLs on Windows
...
class MemoryFilesConfig:
    ...
    soul_path: str = "~/.orion/SOUL.md"
    memory_path: str = "~/.orion/MEMORY.md"
    user_path: str = "~/.orion/USER.md"
...
class SkillsConfig:
    ...
    enabled: bool = True
    skills_dir: str = "~/.orion/skills/"
    active: str = "*"
    auto_discover: bool = True
    auto_sync: bool = False
    nudge_interval: int = 15
    index_repo: str = "https://github.com/orion/skill-index.git"
    index_dir: str = "~/.orion/skill-index/"
    max_depth: int = 5
...
@dataclass
class OrionConfig:
    """Top-level configuration for Orion."""
...
# Sections that users may set via ``orion config set``.
# ``hardware`` is auto-detected and not user-settable.
_SETTABLE_SECTIONS = frozenset(OrionConfig.__dataclass_fields__.keys()) - {
    "hardware",
    "mining",
}
...
def validate_config_key(dotted_key: str) -> type:
    ...
    The function walks the ``OrionConfig`` dataclass hierarchy using
    ``dataclasses.fields()``.
    ...
    for i, part in enumerate(parts):
        ...
        if isinstance(fld_type, str):
            ...
            import orion.core.config as _cfg_mod
...
def _migrate_toml_data(data: Dict[str, Any], cfg: "OrionConfig") -> None:
...
def load_config(path: Optional[Path] = None) -> OrionConfig:
    ...
    ----------
    path:
        Explicit config file. If not set, uses ``OPENORION_CONFIG`` when set,
        otherwise ``~/.orion/config.toml``.
    """
    _ensure_config_dir()
    hw = detect_hardware()
    cfg = OrionConfig(hardware=hw)
    cfg.engine.default = recommend_engine(hw)

    if path is not None:
        config_path = Path(path)
    elif os.environ.get("OPENORION_CONFIG"):
        config_path = Path(os.environ["OPENORION_CONFIG"]).expanduser().resolve()
    else:
...
# ---------------------------------------------------------------------------
# Default TOML generation (for ``orion init``)
# ---------------------------------------------------------------------------
...
# Full reference config: orion init --full
...
def generate_default_toml(
    ...
    """Render a commented TOML string suitable for ``~/.orion/config.toml``."""
    engine = engine or recommend_engine(hw)
    model = recommend_model(hw, engine)
    gpu_line = ""
    if hw.gpu:
...
# enabled = false
# poll_interval = 60
# db_path = ""                # Defaults to ~/.orion/scheduler.db

# [channel.whatsapp_baileys]
# auth_dir = ""               # Defaults to ~/.orion/whatsapp_auth
