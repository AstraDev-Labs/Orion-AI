"""Single point of contact between Python and the Rust ``orion_rust`` module.

Every Python module that wants to delegate to Rust should import helpers from
here rather than importing ``orion_rust`` directly.  The Rust backend is
...
def get_rust_module() -> _types.ModuleType:
    ...
    """
    import orion_rust  # type: ignore[import-untyped]
...
def injection_result_from_json(json_str: str) -> object:
    ...
    from orion.security.injection_scanner import (
        InjectionScanResult,
    )
    from orion.security.types import ScanFinding, ThreatLevel
...
def retrieval_results_from_json(json_str: str) -> list:
    ...
    from orion.tools.storage._stubs import RetrievalResult
