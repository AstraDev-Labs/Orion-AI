"""Config-driven fluent builder that wires up a OrionSystem."""
...
from orion.core.events import EventBus, get_event_bus
...
class SystemBuilder:
    """Config-driven fluent builder for OrionSystem."""
    ...
    def build(self) -> OrionSystem:
        """Construct a fully wired OrionSystem."""
        config = self._config
        ...
        if telemetry_enabled and config.telemetry.gpu_metrics:
            try:
                from orion.telemetry.energy_monitor import (
        ...
        if config.agent_manager.enabled:
            try:
                ...
                am_db = config.agent_manager.db_path or str(
                    Path("~/.orion/agents.db").expanduser()
                )
    ...
    def _resolve_engine(self, config: OrionConfig):
    ...
    def _resolve_model(self, config: OrionConfig, engine: InferenceEngine) -> str:
        if self._model:
            return self._model
    ...
    def _resolve_channel(self, config, bus):
        ...
        try:
            import orion.channels  # noqa: F401 -- trigger registration
            from orion.core.registry import ChannelRegistry
            from orion.system._channel_kwargs import build_channel_kwargs
    ...
    @staticmethod
    def _setup_learning_orchestrator(config: OrionConfig):
        if not config.learning.training_enabled:
            return None
        try:
            from orion.core.config import DEFAULT_CONFIG_DIR
            from orion.learning.learning_orchestrator import (
                LearningOrchestrator,
            )
            from orion.learning.training.lora import LoRATrainingConfig
            from orion.traces.store import TraceStore
