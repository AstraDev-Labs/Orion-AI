...
_BASE_ENV_KEYS = ("PATH", "HOME", "USER", "LANG", "TERM",
                  "SystemRoot", "COMSPEC", "SYSTEMDRIVE", "TEMP", "TMP",
...
class ShellExecTool(BaseTool):
    ...
    def execute(self, **params: Any) -> ToolResult:
        ...
        import sys
        if sys.platform == "win32" and command.strip().lower().startswith("start "):
            try:
                import subprocess
                cmd_clean = command.strip()
                if cmd_clean.lower().startswith("start "):
                    cmd_clean = cmd_clean[6:].strip()
                if cmd_clean.startswith('""'):
                    cmd_clean = cmd_clean[2:].strip()
                ...
                try:
                    p.wait(timeout=0.5)
                    if p.returncode != 0:
                        stderr_content = p.stderr.read().strip() if p.stderr else ""
                        return ToolResult(
                            tool_name="shell_exec",
                            content=stderr_content or f"Command '{cmd_clean}' failed to start.",
                            success=False,
                        )
                except subprocess.TimeoutExpired:
                    # Process is running in the background successfully!
                    pass

                return ToolResult(
                    tool_name="shell_exec",
                    content="Process started successfully in background.",
                    success=True,
                )
