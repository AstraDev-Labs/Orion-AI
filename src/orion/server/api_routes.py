...
try:
    from orion.server.dashboard_routes import dashboard_router
except ImportError:
...
async def list_agents(request: Request):
    ...
    registered = []
    try:
        import orion.agents  # noqa: F401 — side-effect registration
    ...
    running = []
    try:
        from orion.tools.agent_tools import _SPAWNED_AGENTS
...
async def create_agent(req: AgentCreateRequest, request: Request):
    """Spawn a new agent."""
    try:
        from orion.tools.agent_tools import AgentSpawnTool
...
async def kill_agent(agent_id: str, request: Request):
    ...
    try:
        from orion.tools.agent_tools import AgentKillTool
...
async def message_agent(agent_id: str, req: AgentMessageRequest, request: Request):
    ...
    try:
        from orion.tools.agent_tools import AgentSendTool
...
async def list_skills(request: Request):
    ...
    try:
        from orion.core.registry import SkillRegistry
...
async def install_skill(request: Request):
    ...
    return {
        ...
        "message": "Use TOML files in ~/.orion/skills/",
    }
...
async def list_sessions(request: Request, limit: int = 20):
    ...
    try:
        from orion.sessions.store import SessionStore
...
async def prometheus_metrics(request: Request):
    ...
    try:
        ...
        lines = [
            "# HELP orion_requests_total Total requests processed",
            "# TYPE orion_requests_total counter",
            f"orion_requests_total {stats.get('total_requests', 0)}",
            "# HELP orion_tokens_total Total tokens generated",
            "# TYPE orion_tokens_total counter",
            f"orion_tokens_total {stats.get('total_tokens', 0)}",
            "# HELP orion_latency_avg_ms Average latency in milliseconds",
            "# TYPE orion_latency_avg_ms gauge",
            f"orion_latency_avg_ms {stats.get('avg_latency_ms', 0)}",
        ]
        from starlette.responses import PlainTextResponse
...
async def learning_stats(request: Request):
    ...
    # Skill discovery
    try:
        from orion.learning.agents.skill_discovery import SkillDiscovery
...
def include_all_routes(app) -> None:
    ...
    try:
        if hasattr(app.state, "agent_manager") and app.state.agent_manager:
            from orion.server.agent_manager_routes import (  # noqa: PLC0415
    ...
    try:
        ...
        from orion.server.ws_bridge import create_ws_router
