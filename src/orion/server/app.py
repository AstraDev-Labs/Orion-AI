"""FastAPI application factory for the Orion API server."""
        ...
        for agent in mgr.list_agents():
            ...
            for b in bindings:
                ...
                if bridge and hasattr(bridge, "_channels"):
                    ...
                    from orion.server.channel_bridge import ChannelBridge
                    from orion.server.session_store import SessionStore
...
def create_app(
    ...
    *,
    ...
    config:
        Optional OrionConfig for other settings.
    """
    app = FastAPI(
        title="Orion API",
        description="OpenAI-compatible API server for Orion",
        version="0.1.0",
    )
    ...
    try:
        from orion.server.middleware import create_security_middleware
    ...
    if api_key:
        try:
            from orion.server.auth_middleware import AuthMiddleware
    ...
    if webhook_config:
        try:
            from orion.server.webhook_routes import (
    ...
    # Serve static frontend assets from src/orion/server/static directory
