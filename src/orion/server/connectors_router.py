...
def _ensure_connectors_registered() -> None:
    ...
    try:
        import orion.connectors  # noqa: F401
    except Exception:
        pass
    ...
    if not ConnectorRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("orion.connectors.")
    ...
    def _start_sync(connector_id: str, instance: Any) -> str:
        ...
        def _run_sync() -> None:
            try:
                ...
                if getattr(instance, "auth_type", "") == "filesystem":
                    try:
                        from orion.cli.connect_cmd import (
                            _index_connector_into_memory,
                        )

                        chunks = _index_connector_into_memory(instance, connector_id)
                        logger.info(
                            "Indexed %s %s chunks into chat memory",
                            chunks,
                            connector_id,
                        )
                    except Exception as exc:  # noqa: BLE001
                        logger.warning(
                            "Chat memory indexing failed for %s: %s",
                            connector_id,
                            exc,
                        )
            ...
            if auth_type == "filesystem":
                ...
                if req.path:
                    ...
                    if instance._connected:
                        ...
                        config_path = (
                            DEFAULT_CONFIG_DIR / "connectors" / f"{connector_id}.json"
                        )
                        config_path.parent.mkdir(parents=True, exist_ok=True)
                        config_path.write_text(
                            json.dumps(
                                {"path": req.path, "vault_path": req.path},
                                indent=2,
                            ),
                            encoding="utf-8",
                        )
    ...
    async def oauth_callback(
        ...
        return HTMLResponse(
            content=(
                ...
                "<p>You can close this tab and return to Orion.</p>"
