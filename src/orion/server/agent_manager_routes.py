...
logger = logging.getLogger("orion.server.agent_manager")
...
class _LightweightSystem:
    ...
    full OrionSystem (which picks a random model from Ollama)."""
...
def _make_lightweight_system(
    ...
    try:
        ...
        try:
            from orion.core.events import get_event_bus
            from orion.telemetry.instrumented_engine import (
                InstrumentedEngine,
            )
...
def _ensure_registries_populated() -> None:
    ...
    from orion.core.registry import ChannelRegistry, ToolRegistry

    # First, try a normal import (works if modules haven't been imported yet)
    try:
        import orion.channels  # noqa: F401
    except Exception:
        pass
    ...
    # Also try to import browser tools (not included in orion.tools.__init__)
    for _browser_mod in ("orion.tools.browser", "orion.tools.browser_axtree"):
        try:
    ...
    if not ChannelRegistry.keys():
        for mod_name in list(sys.modules):
            if mod_name.startswith("orion.channels.") and not mod_name.endswith(
                "_stubs"
            ):
                try:
    ...
    if not ToolRegistry.keys():
        for mod_name in list(sys.modules):
            if (
                mod_name.startswith("orion.tools.")
...
def _build_deep_research_tools(
    ...
    from orion.connectors.retriever import TwoStageRetriever
    from orion.connectors.store import KnowledgeStore
    from orion.tools.knowledge_search import KnowledgeSearchTool
    from orion.tools.knowledge_sql import KnowledgeSQLTool
    from orion.tools.scan_chunks import ScanChunksTool
    from orion.tools.think import ThinkTool
...
def _get_mcp_tools(app_state: Any) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    ...
    from orion.mcp.transport import StdioTransport, StreamableHTTPTransport
    from orion.tools.mcp_adapter import MCPToolProvider
    ...
    if agent_type == "deep_research" and app_state is not None:
        ...
        if dr_tools:
            ...
            async def generate_deep_research():
                ...
                from orion.agents.deep_research import DeepResearchAgent
    ...
    async def generate():
        ...
        while turns < max_turns:
            ...
            if tool_call_fragments and current_finish_reason == "tool_calls":
                ...
                from orion.core.types import ToolCall as MsgToolCall
                ...
                for tc in sorted_tcs:
                    ...
                    try:
                        ...
                        if mcp_adapter is not None:
                            ...
                            from orion.tools._stubs import (
...
def create_agent_manager_router(
    ...
    async def bind_channel(
        ...
        if req.channel_type == "imessage":
            ...
            if identifier:
                try:
                    from orion.channels.imessage_daemon import (
                        is_running,
                        run_daemon,
                    )
                    ...
                    if not is_running():
                        ...
                        if engine:
                            from orion.server.agent_manager_routes import (
                                _build_deep_research_tools,
                            )

                            tools = _build_deep_research_tools(engine=engine, model="")
                            if tools:
                                from orion.agents.deep_research import (
                                    DeepResearchAgent,
                                )
        ...
        if req.channel_type == "sendblue":
            ...
            if api_key_id and api_secret_key:
                try:
                    from orion.channels.sendblue import (
                        SendBlueChannel,
                    )
                    ...
                    if bridge and hasattr(bridge, "_channels"):
                        ...
                        from orion.server.channel_bridge import (
                            ChannelBridge,
                        )
                        from orion.server.session_store import (
                            SessionStore,
                        )
        ...
        if req.channel_type == "slack":
            ...
            if bot_token and app_token:
                try:
                    from orion.channels.slack_daemon import (
                        start_slack_daemon,
                    )
                    from orion.channels.slack_daemon import (
    ...
    def list_traces(agent_id: str, limit: int = 20):
        ...
        try:
            ...
            config = load_config()
            store = TraceStore(config.traces.db_path or "~/.orion/traces.db")
    ...
    def credential_status(tool_name: str):
        from orion.core.credentials import get_credential_status
