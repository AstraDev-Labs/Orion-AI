...
Recipes are the universal composition format for Orion.  Each recipe
...
_USER_RECIPES_DIR = Path.home() / ".orion" / "recipes"
_USER_OPERATORS_DIR = Path.home() / ".orion" / "operators"
...
class Recipe:
    ...
    def to_builder_kwargs(self) -> Dict[str, Any]:
        """Convert recipe fields to kwargs for SystemBuilder/Orion.

        Returns a dict with only the non-None fields, keyed to match
        the SystemBuilder fluent API or Orion constructor parameters.
        """
        kwargs: Dict[str, Any] = {}
    ...
    def to_eval_suite(
        ...
        """
        from orion.recipes.composer import recipe_to_eval_suite
    ...
    def to_operator_manifest(self) -> Any:
        ...
        from orion.recipes.composer import recipe_to_operator
...
def discover_recipes(
    ...
    *,
    ...
    3. User ``~/.orion/recipes/`` directory
    4. User ``~/.orion/operators/`` directory
