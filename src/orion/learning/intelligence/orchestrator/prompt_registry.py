...
# ---------------------------------------------------------------------------
# Tool descriptions for Orion built-in tools
# ---------------------------------------------------------------------------
...
TOOL_DESCRIPTIONS: Dict[str, dict] = {
    ...
    "play_music": {
        "category": "media",
        "description": (
            "PLAY_MUSIC - Play music on a platform\n"
            "  - BEST FOR: Playing songs, playlists, mood-based music\n"
            "  - STRENGTHS: Supports Spotify, YouTube Music, YouTube, Apple Music, local media player\n"
            "  - USE WHEN: User wants to play, listen to, or start music\n"
            "  - COST: Free\n"
            '  - Input: JSON with keys: platform, song, artist, mood, language, genre\n'
            '  - CRITICAL: If the user asks for music but does NOT name a specific song, YOU MUST LEAVE "song" EMPTY.\n'
            '  - Example: {"platform":"youtube_music","song":"Blinding Lights","artist":"The Weeknd"}\n'
            '  - Example: {"platform":"apple_music"}'
        ),
        "examples": [
            {
                "task": "Play some happy Tamil songs on Spotify",
                "thought": "Use play_music.",
                "input": '{"platform":"spotify","mood":"happy","language":"tamil"}',
            },
            {
                "task": "Can you play a song in Apple Music?",
                "thought": "No song specified.",
                "input": '{"platform":"apple_music"}',
            },
            {
                "task": "Play some relaxing music",
                "thought": "User wants relaxing music - use play_music with mood.",
                "input": '{"mood":"relaxed"}',
            },
        ],
    },
    "play_video": {
        "category": "utility",
        "description": (
            "PLAY_VIDEO - Play or search for TV shows and movies on streaming platforms\n"
            "  - BEST FOR: Watching TV series, movies, anime, or opening streaming services\n"
            "  - STRENGTHS: Supports Netflix, Apple TV, Prime Video, Disney+, Hulu, Crunchyroll, YouTube\n"
            "  - USE WHEN: User asks to watch a movie, show, or open a streaming app\n"
            "  - COST: Free\n"
            '  - Input: JSON with keys: platform, title, genre\n'
            '  - Example: {"platform":"netflix","title":"Stranger Things"}\n'
            '  - Example: {"platform":"apple_tv","title":"Ted Lasso"}'
        ),
        "examples": [
            {
                "task": "Put on Stranger Things on Netflix",
                "thought": "User wants to watch a show on Netflix - use play_video.",
                "input": '{"platform":"netflix","title":"Stranger Things"}',
            },
            {
                "task": "Play Ted Lasso on Apple TV",
                "thought": "User wants to watch a show on Apple TV - use play_video.",
                "input": '{"platform":"apple_tv","title":"Ted Lasso"}',
            },
        ],
    },
    # Obsidian tools
    "obsidian_search_notes": {
        "category": "memory",
        "description": (
            "OBSIDIAN_SEARCH_NOTES - Search the Obsidian knowledge vault\n"
            "  - BEST FOR: Recalling past memories, preferences, and stored knowledge\n"
            "  - STRENGTHS: Semantic search over markdown notes in Obsidian\n"
            "  - USE WHEN: Need to recall something from the user's knowledge base\n"
            "  - COST: Free (local)\n"
            '  - Input: JSON with key "query", e.g. {"query":"user preferences"}'
        ),
        "examples": [],
    },
    "obsidian_write_note": {
        "category": "memory",
        "description": (
            "OBSIDIAN_WRITE_NOTE - Write a note to the Obsidian vault\n"
            "  - BEST FOR: Storing new memories, facts, or context permanently\n"
            "  - STRENGTHS: Persistent markdown notes the user can browse\n"
            "  - USE WHEN: Need to remember something for later\n"
            "  - COST: Free (local)\n"
            '  - Input: JSON with keys "title" and "content"'
        ),
        "examples": [],
    },
    # System tools
    "shell_exec": {
        ...
        "description": (
            ...
            "  - BEST FOR: Opening apps, running scripts, system automation\n"
            "  - STRENGTHS: Full system access, can open any Windows app\n"
            "  - USE WHEN: Need to run a command, open an app, check system info\n"
            "  - COST: Free (local execution)\n"
            '  - Input: JSON with key "command", e.g. {"command":"start chrome"}'
        ),
        "examples": [
            {
                "task": "Open Google Chrome",
                "thought": "Need to open an app - use shell_exec.",
                "input": '{"command":"start chrome"}',
            },
            {
                "task": "What time is it?",
                "thought": "Need current time - use shell_exec with PowerShell.",
                "input": '{"command":"powershell -Command Get-Date"}',
            },
        ],
    },
}
...
def build_system_prompt(
    ...
    *,
    ...
    if tools is not None:
        from orion.tools._stubs import build_tool_descriptions
