...
The wrapper adapts Orion traces into ACE's
...
sidecar overlay under ``~/.orion/learning/ace/<task>/`` for the
...
from orion.core.config import ACEOptimizerConfig
...
def _default_save_dir(task_name: str) -> Path:
    return Path.home() / ".orion" / "learning" / "ace" / task_name


class _TraceDataProcessor:
    """Adapter that exposes Orion traces in ACE's three-method API.
...
def _traces_to_samples(traces: List[Any]) -> List[Dict[str, Any]]:
    """Convert Orion trace records to ACE's sample dict shape."""
