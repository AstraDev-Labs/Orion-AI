...
Uses GEPA's adapter pattern to bridge Orion traces into GEPA's
...
from orion.core.config import GEPAOptimizerConfig
from orion.core.registry import LearningRegistry
from orion.learning._stubs import AgentLearningPolicy
...
class OrionGEPAAdapter:
    """Implements GEPA's adapter protocol for Orion agents.
...
class GEPAAgentOptimizer:
    ...
    def optimize(self, trace_store: Any) -> Dict[str, Any]:
        ...
        adapter = OrionGEPAAdapter(trace_store, agent_name, self.config)
        adapter.load_traces()
...
__all__ = ["GEPAAgentOptimizer", "OrionGEPAAdapter"]
