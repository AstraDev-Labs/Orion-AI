...
must NEVER be inside the Orion source tree. ``resolve_spec_search_root``
walks up from this module's ``__file__`` looking for a ``pyproject.toml`` that
identifies the Orion source root, then refuses to operate if the resolved
root is inside it. Defense in depth — if a user accidentally points
``OPENORION_HOME`` at the repo, the system fails loudly instead of silently
...
def _find_source_root() -> Path | None:
    """Walk upward from this module to find the Orion source root.

    Returns the directory containing the Orion ``pyproject.toml``, or
    ...
    for candidate in (here, *here.parents):
        ...
        if py.exists():
            try:
                ...
                continue
            if 'name = "orion"' in content.lower():
                return candidate
    return None


def _resolve_orion_home() -> Path:
    """Resolve the OPENORION_HOME directory (env var or default)."""
    env = os.environ.get("OPENORION_HOME")
    if env:
        return Path(env).expanduser().resolve()
    return (Path.home() / ".orion").resolve()


def resolve_spec_search_root() -> Path:
    """Return the absolute path of the spec-search root directory.

    The root is ``$OPENORION_HOME/learning`` (or ``~/.orion/learning``
    by default). Raises ``ConfigurationError`` if the resolved path lies
    inside the Orion source tree, to prevent dev artifacts from leaking
    into the repo.
    """
    home = _resolve_orion_home()
    ...
    if source_root is not None:
        try:
            ...
            raise ConfigurationError(
                f"OPENORION_HOME ({home}) is inside the source tree "
                f"({source_root}). Spec search refuses to write runtime "
                "artifacts inside the Orion repo. Set OPENORION_HOME "
                "to a directory outside the repo (default: ~/.orion)."
            )
