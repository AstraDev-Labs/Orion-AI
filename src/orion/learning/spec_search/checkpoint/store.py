...
A thin wrapper over a local git repository at ``<orion_home>/.git``.
...
from orion.learning.spec_search.storage.paths import (
    ConfigurationError,
    _find_source_root,
)
...
class CheckpointStore:
    ...
    root :
        ...
        ``~/.orion/`` (or ``$OPENORION_HOME``); for tests it's a
    ...
    def init(self) -> None:
        ...
        Refuses to initialize if ``self.root`` is inside the Orion source
        ...
        if source_root is not None:
            try:
                ...
                raise ConfigurationError(
                    ...
                    f"Orion source tree ({source_root}). Refusing to "
        ...
        if not (self._root / ".git").exists():
            self._git("init", "-q")
            self._git("config", "user.email", "spec-search@orion.local")
            self._git("config", "user.name", "Orion Spec Search")
        ...
        # Allow empty so init succeeds even on a brand-new orion home.
        self._git(
            "commit",
            "--allow-empty",
            "-q",
