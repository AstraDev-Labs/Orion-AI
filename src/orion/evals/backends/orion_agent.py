"""Orion Agent backend — agent-level inference with tool calling."""
...
from orion.evals.backends._commit_util import orion_commit
from orion.evals.core.backend import InferenceBackend


class OrionAgentBackend(InferenceBackend):
    """Agent-level inference via SystemBuilder + OrionSystem.ask().

    Supports tool calling via the agent harness. Works for both local
    and cloud models.
    """

    backend_id = "orion-agent"
    framework_name = "orion"
    ...
    def __init__(
        ...
        # Override the agent's per-run turn budget. OrionConfig.agent.max_turns
    ...
    def framework_commit_value(self) -> str:
        """Orion repo HEAD commit (for telemetry tagging)."""
        from orion.evals.backends._commit_util import orion_commit
