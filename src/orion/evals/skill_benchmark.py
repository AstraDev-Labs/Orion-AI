...
class SkillBenchmarkConfig:
    ...
    skills_dir: Path = field(
        default_factory=lambda: Path("~/.orion/skills/").expanduser()
    )
    overlay_dir_dspy: Path = field(
        default_factory=lambda: Path("~/.orion/learning/skills-dspy/").expanduser()
    )
    overlay_dir_gepa: Path = field(
        default_factory=lambda: Path("~/.orion/learning/skills-gepa/").expanduser()
    )
...
class SkillBenchmarkRunner:
    ...
    def __init__(self, config: SkillBenchmarkConfig) -> None:
        ...
        # the default ~/.orion/learning/skills/ tree.
    ...
    def _backend_kwargs_for_condition(self, condition: str) -> Dict[str, Any]:
        """Return the kwargs to pass to OrionAgentBackend for *condition*.
    ...
    def _build_backend_for_condition(self, condition: str) -> Any:
        """Construct a OrionAgentBackend for *condition*.
    ...
    def _run_single_seed(
        ...
        try:
            judge_backend = OrionDirectBackend(
                engine_key=self._config.engine,
            )
