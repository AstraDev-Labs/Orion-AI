...
from orion.evals.core.config import EvalConfigError, expand_suite, load_eval_config
...
class TestExpandSuite:
    ...
    def test_backend_from_benchmark(self):
        suite = EvalSuiteConfig(
            models=[ModelConfig(name="m1")],
            benchmarks=[BenchmarkConfig(name="gaia", backend="orion-agent")],
        )
        configs = expand_suite(suite)
        assert configs[0].backend == "orion-agent"
...
class TestCLIConfig:
    def test_run_missing_benchmark_and_config(self):
        ...
        from orion.evals.cli import main
    ...
    def test_run_config_loads_and_prints_suite_info(self, tmp_path):
        ...
        runner = CliRunner()
        with patch("orion.evals.cli._run_single", side_effect=Exception("mock")):
