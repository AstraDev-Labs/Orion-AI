...
from orion.evals.core.backend import InferenceBackend
from orion.evals.core.dataset import DatasetProvider
from orion.evals.core.export import _hardware_info_dict
from orion.evals.core.scorer import Scorer
from orion.evals.core.tracker import ResultTracker
from orion.evals.core.types import (
    EvalRecord,
    EvalResult,
    MetricStats,
    RunConfig,
...
try:
    from orion.telemetry.efficiency import compute_efficiency
except ImportError:  # pragma: no cover
    compute_efficiency = None  # type: ignore[assignment]

try:
    from orion.telemetry.efficiency import estimate_model_flops_per_token
except ImportError:  # pragma: no cover
...
class EvalRunner:
    ...
    def run(
        ...
        from orion.evals.core.dataset import DatasetProvider as _DP
    ...
    def _process_one(self, record: EvalRecord) -> EvalResult:
        ...
        def _backend_error_result(full: dict, message: str) -> EvalResult:
            ...
            return EvalResult(
                ...
                framework=full.get(
                    "framework",
                    getattr(self._backend, "framework_name", "orion"),
                ),
        ...
        try:
            ...
            return EvalResult(
                ...
                framework=full.get("framework", "orion"),
            ...
            return EvalResult(
                ...
                model_answer="",
                error=str(exc),
                framework=getattr(self._backend, "framework_name", "orion"),
