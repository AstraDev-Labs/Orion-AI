...
``ORION_ALLOW_COMMIT_DRIFT=1`` disables strict commit-pin enforcement.
"""
...
# python so its deps don't need to be installed in Orion's venv); for
...
def verify_commit_pin(entry: ThirdPartyEntry) -> None:
    ...
    Raises:
        ...
        CommitDriftError: if HEAD doesn't match pinned_commit, unless
            ``ORION_ALLOW_COMMIT_DRIFT=1`` is set (then logs a warning).
    """
    ...
    if not actual.startswith(pinned) and not pinned.startswith(actual):
        msg = (
            ...
            f"To bypass for this run, set ORION_ALLOW_COMMIT_DRIFT=1."
        )
        if os.environ.get("ORION_ALLOW_COMMIT_DRIFT") == "1":
            LOGGER.warning(msg)
            return
