...
::
    ...
    python -m orion.agents.hybrid.runner --cell minions-gaia-qwen27b-opus-3

Reads a cell definition from ``registry/<method>.toml`` (bundled with this
package or pointed at by ``OPENORION_HYBRID_REGISTRY_DIR``), constructs
the registered agent, loads bench tasks via Orion's existing dataset
...
existing rescore / dashboard scripts can read Orion cells without
modification.
"""
...
from orion.agents.hybrid._prompts import format_prompt as _format_prompt

PACKAGE_DIR = Path(__file__).parent
DEFAULT_REGISTRY_DIR = PACKAGE_DIR / "registry"
DEFAULT_EXPERIMENTS_DIR = Path(
    os.environ.get(
        "OPENORION_HYBRID_EXPERIMENTS_DIR",
        str(Path.home() / ".orion-hybrid" / "experiments"),
    )
)
...
def load_registry(registry_dir: Optional[Path] = None) -> Dict[str, Dict[str, Any]]:
    ...
    env_override = os.environ.get("OPENORION_HYBRID_REGISTRY_DIR")
    if env_override:
...
def _score_gaia(task: Dict[str, Any], answer: str) -> Dict[str, Any]:
    ...
    against the reference. Use the Orion gaia_exact scorer for the
...
def _score_swebench(task: Dict[str, Any], answer: str) -> Dict[str, Any]:
    ...
    from orion.evals.scorers.swebench_harness import (
...
def _build_agent(cell: Dict[str, Any]):
    ...
    import orion.agents  # noqa: F401 — populate registry
...
def main(argv: Optional[List[str]] = None) -> int:
    p = argparse.ArgumentParser(
        prog="python -m orion.agents.hybrid.runner",
