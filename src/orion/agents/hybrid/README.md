...
registered as a standard Orion agent so the rest of the platform
...
Items in parentheses are what the *paper* trains. These Orion ports
...
`src/orion/evals/scorers/swebench_harness.py` (next to the existing
structural scorer).

## Quickstart

```bash
cd /matx/u/aspark/Orion
source .env                                           # API keys
...
# 3. Run a smoke cell
.venv/bin/python -m orion.agents.hybrid.runner \
    --cell minions-gaia-qwen27b-opus-3
```

Outputs land in
`$OPENORION_HYBRID_EXPERIMENTS_DIR/<cell>/{results.jsonl,summary.json,config.json,logs/}`
(defaults to `~/.orion-hybrid/experiments/`). The schema matches the
...
```bash
src/orion/agents/hybrid/scripts/new_experiment.sh \
...
these Orion cells should reproduce the harness numbers within
