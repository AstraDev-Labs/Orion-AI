...
from orion.agents.errors import (
    AgentTickError,
    EscalateError,
    FatalError,
    classify_error,
...
class AgentExecutor:
    ...
    Constructor receives a OrionSystem reference for access to engine,
    ...
    def set_system(self, system: Any) -> None:
        """Deferred system injection — called after OrionSystem is constructed."""
        self._system = system
    ...
    def _invoke_agent(self, agent: dict) -> AgentResult:
        ...
        from orion.agents import AgentRegistry
        ...
        # Resolve engine + model from OrionSystem
        engine = self._system.engine if self._system else None
        if engine is None:
            raise FatalError("No engine available in OrionSystem")
        ...
        if router_policy_key and self._system:
            try:
                from orion.core.registry import RouterPolicyRegistry
        ...
        # agent's own ToolExecutor. Set by CLI paths like `orion agents ask`
    ...
    def _save_trace(
        ...
        from orion.core.types import StepType, Trace, TraceStep
