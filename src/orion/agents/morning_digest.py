...
from orion.agents.digest_store import DigestArtifact, DigestStore
from orion.core.registry import AgentRegistry
from orion.core.types import Message, Role, ToolCall


def _load_persona(persona_name: str) -> str:
    """Load a persona prompt file by name."""
    search_paths = [
        Path("configs/orion/prompts/personas") / f"{persona_name}.md",
        Path.home() / ".orion" / "prompts" / "personas" / f"{persona_name}.md",
    ]
...
class MorningDigestAgent(ToolUsingAgent):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self._persona = kwargs.pop("persona", "orion")
        ...
        # Force local free kokoro engine, ignoring any database preset overrides
        kwargs.pop("tts_backend", None)
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        evaluator_feedback = ""
        try:
            from orion.agents.digest_evaluator import DigestEvaluator
        ...
        output_dir = str(Path.home() / ".orion" / "digests")
        tts_call = ToolCall(
            id="digest-tts-1",
            name="text_to_speech",
