...
# Ensure we can import orion
sys.path.append(os.path.dirname(os.path.abspath(__file__)))

import orion
from orion_vision import FaceAuthenticator
...
# Keep face auth enabled at runtime so the auth loop always starts.
...
async def lifespan(app: FastAPI):
    # Startup Sequence
    import sys
    print(f"[SERVER DEBUG] Startup Sequence: INITIATED")
    print(f"[SERVER DEBUG] Python Version: {sys.version}")
    
    print("[SERVER] Initializing Kasa Agent...")
    await kasa_agent.initialize()
    
    yield
    # Shutdown Sequence (Add cleanup logic here if needed)
    print(f"[SERVER DEBUG] Shutdown Sequence: COMPLETED")
...
async def connect(sid, environ):
    ...
    await sio.emit('status', {'msg': 'Connected to ORION'}, room=sid)
    ...
    async def on_auth_metrics(metrics):
        await sio.emit('auth_metrics', metrics)
    ...
    if authenticator is None:
        ...
        authenticator = FaceAuthenticator(
            ...
            on_metrics=on_auth_metrics,
            camera_flipped=SETTINGS.get("camera_flipped", False)
        )
    ...
    if authenticator.authenticated:
        ...
        print("[SERVER] Starting face authentication loop.")
...
@sio.on('auth_frame_upload')
async def handle_auth_frame(sid, data):
    if authenticator:
        await authenticator.process_external_image(data.get('image'))
...
async def start_audio(sid, data=None):
    ...
    audio_mode = SETTINGS.get("audio_mode", "speaker")
    if data:
        ...
        if 'audio_mode' in data and data['audio_mode']:
            audio_mode = data['audio_mode']
            
    print(f"Using input device: Index={device_index}, Output device: Index={output_device_index}")
    ...
    try:
        ...
        audio_loop.output_device_name = output_device_name
        ...
        print(f"CRITICAL ERROR STARTING ORION: {e}")
        import traceback
        traceback.print_exc()
...
async def user_input(sid, data):
    ...
    if text:
        ...
        asyncio.create_task(audio_loop.process_text_input(text))
        print(f"[SERVER DEBUG] Message sent to local model successfully.")

@sio.event
async def speak_welcome(sid, data=None):
    if not audio_loop:
        print("[SERVER DEBUG] Cannot speak welcome: audio loop inactive.")
        return
    message = "Online and ready, sir. ORION systems nominal."
    audio_loop._emit_transcription("ORION", message)
    await audio_loop.audio_queue.put(message)
...
async def video_frame(sid, data):
    # legacy multimodal stream frame - ignored in local mode as we use mss
    pass
...
async def upload_memory(sid, data):
    ...
    try:
        ...
        context_msg = f"System Notification: The user has uploaded a long-term memory file. Please load the following context into your understanding:\n\n{memory_text}"
        
        asyncio.create_task(audio_loop.process_text_input(context_msg))
        print("Memory context sent to local reasoning loop.")
...
async def get_settings(sid):
    ...
    settings_copy["face_auth_enabled"] = True
    await sio.emit('settings', settings_copy)
...
async def update_settings(sid, data):
    ...
    if "audio_mode" in data:
        SETTINGS["audio_mode"] = data["audio_mode"]
        print(f"[SERVER] Audio mode set to: {data['audio_mode']}")
        if audio_loop:
            audio_loop.audio_mode = data["audio_mode"]
