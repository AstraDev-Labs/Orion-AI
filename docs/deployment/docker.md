...
Orion provides Docker images for both CPU-only and GPU-accelerated deployments, along with a Docker Compose configuration that bundles the API server with an Ollama inference backend.

## Quick Start

The fastest way to get Orion running in Docker is with Docker Compose, which starts both the API server and an Ollama backend:
...
1. **Builder stage** -- installs `uv` and the `orion[server]` package (which includes FastAPI, uvicorn, and all server dependencies) from the project source.
...
```bash
docker run -d -p 8000:8000 orion:latest
```
...
```bash
docker build -f Dockerfile.gpu -t orion:gpu .
```
...
```bash
docker run -d --gpus all -p 8000:8000 orion:gpu
```
...
The `docker-compose.yml` defines a complete deployment with the Orion API server and an Ollama backend:
...
services:
  orion:
    ...
    ports:
      - "8000:8000"
    environment:
      - OPENORION_ENGINE_DEFAULT=ollama
      - OPENORION_OLLAMA_HOST=http://ollama:11434
    depends_on:
      - ollama
...
The `orion` service is configured through environment variables:

| Variable                      | Description                                             | Default                    |
|-------------------------------|---------------------------------------------------------|----------------------------|
| `OPENORION_ENGINE_DEFAULT`   | Inference engine backend to use                         | `ollama`                   |
| `OPENORION_OLLAMA_HOST`      | URL of the Ollama server (uses Docker service name)     | `http://ollama:11434`      |
...
The `orion` service declares `depends_on: ollama`, ensuring the Ollama container starts before the API server. Both services use `restart: unless-stopped` to automatically recover from crashes.

## Custom Configuration

### Mounting a Configuration File

To use a custom `config.toml`, mount it into the container at the expected path (`~/.orion/config.toml`, which is `/root/.orion/config.toml` in the container):
...
services:
  orion:
    ...
    ports:
      - "8000:8000"
    volumes:
      - ./my-config.toml:/root/.orion/config.toml:ro
    environment:
      - OPENORION_ENGINE_DEFAULT=ollama
      - OPENORION_OLLAMA_HOST=http://ollama:11434
    depends_on:
...
To persist telemetry data, memory databases, and trace records across container restarts, mount the entire Orion data directory:
...
The entrypoint is `orion` and the default command is `serve --host 0.0.0.0 --port 8000`. Override the command to change server options:

```bash
docker run -d -p 9000:9000 orion:latest \
...
### Available CLI Options for `orion serve`
