...
Orion includes a launchd property list (plist) for running the API server as a background service on macOS. This provides automatic startup at login, automatic restart if the process exits, and log capture.
...
```bash
cp deploy/launchd/com.orion.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.orion.plist
```
...
You should see a line with the PID and the label `com.orion`. A `0` in the status column indicates the service is running normally.
...
The provided plist file at `deploy/launchd/com.orion.plist`:
...
<dict>
    <key>Label</key>
    <string>com.orion</string>
    <key>ProgramArguments</key>
    <array>
        <string>/usr/local/bin/orion</string>
        <string>serve</string>
        <string>--host</string>
    ...
    <string>/tmp/orion.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/tmp/orion.stderr.log</string>
</dict>
</plist>
```
...
|----------------------|--------------------------------|------------------------------------------------------------------------------------------------------|
| `Label`              | `com.orion`               | Unique identifier for the service. Used with `launchctl` commands to manage the service.             |
| `ProgramArguments`   | `["/usr/local/bin/orion", "serve", "--host", "0.0.0.0", "--port", "8000"]` | The command and arguments to execute. Each element of the command line is a separate string in the array. |
...
| `StandardOutPath`    | `/tmp/orion.stdout.log`   | File where standard output is written. Contains server startup messages and access logs.             |
| `StandardErrorPath`  | `/tmp/orion.stderr.log`   | File where standard error is written. Contains error messages and stack traces.                      |
...
cat /tmp/orion.stdout.log

# View standard error (errors, warnings)
cat /tmp/orion.stderr.log

# Follow logs in real time
tail -f /tmp/orion.stdout.log /tmp/orion.stderr.log
```
...
!!! tip "Persistent log location"
    ...
    ```xml
    <key>StandardOutPath</key>
    <string>/Users/yourname/.orion/orion.stdout.log</string>
    <key>StandardErrorPath</key>
    <string>/Users/yourname/.orion/orion.stderr.log</string>
    ```
...
launchctl unload ~/Library/LaunchAgents/com.orion.plist
```
...
```bash
# Stop the service
launchctl stop com.orion

# Start the service
launchctl start com.orion
```
...
```bash
# List all loaded services matching "orion"
launchctl list | grep orion
```
...
| Label  | The service label (`com.orion`)                           |
...
<dict>
    <key>OPENORION_ENGINE_DEFAULT</key>
    <string>ollama</string>
    <key>OPENORION_OLLAMA_HOST</key>
    <string>http://localhost:11434</string>
</dict>
```

### Using a Different `orion` Binary Path

If `orion` is installed in a virtual environment or a non-standard location, update the first element of `ProgramArguments`:

```xml
<key>ProgramArguments</key>
<array>
    <string>/Users/yourname/.local/bin/orion</string>
    <string>serve</string>
    <string>--host</string>
    <string>0.0.0.0</string>
    <string>--port</string>
...
The instructions above install the service as a **user agent** (runs only when you are logged in). To run Orion as a system-wide daemon that starts at boot regardless of user login:
...
```bash
sudo cp deploy/launchd/com.orion.plist /Library/LaunchDaemons/
sudo chown root:wheel /Library/LaunchDaemons/com.orion.plist
sudo launchctl load /Library/LaunchDaemons/com.orion.plist
```
