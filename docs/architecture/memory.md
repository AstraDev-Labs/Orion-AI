...
- **Persistence:** Data persists across restarts in `~/.orion/memory.db`
...
from orion.tools.storage.faiss_backend import FAISSMemory
from orion.tools.storage.hybrid import HybridMemory
...
!!! note "Backward compatibility"
    The old imports (e.g., `from orion.memory.sqlite import SQLiteMemory`) still work via backward-compatibility shims in the `memory/` package, but the canonical location is now `orion.tools.storage.*`.
...
```python
from orion.tools.storage.chunking import chunk_text, ChunkConfig
...
```python
from orion.tools.storage.embeddings import SentenceTransformerEmbedder
...
```python
from orion.tools.storage.context import inject_context, ContextConfig
...
The default backend is configured in `~/.orion/config.toml`. Storage settings live under `[tools.storage]`, and context injection is controlled by `agent.context_from_memory`:
