...
This page traces the end-to-end journey of a user query through the Orion system, from the moment it enters the CLI or SDK to the final response and telemetry recording.
...
sequenceDiagram
    ...
    User->>CLI: orion ask "query" / j.ask("query")
    CLI->>CFG: load_config()
    CFG-->>CLI: OrionConfig (hardware, engine defaults)
...
Orion supports two query processing paths, selected by the `--agent` CLI flag or the `agent` parameter in the SDK.
...
```bash
# CLI
orion ask --agent orchestrator --tools calculator,think "What is 2^10 + 3^5?"
...
```python
config = load_config()  # Reads ~/.orion/config.toml
```
...
- Returns a `OrionConfig` dataclass with all settings
...
The `TelemetryStore` subscribes to `TELEMETRY_RECORD` events on the EventBus and writes records to `~/.orion/telemetry.db`.
...
The `Orion` class in `sdk.py` provides the same query flow through a Python API:

```python
from orion import Orion

j = Orion(model="qwen3:8b", engine_key="ollama")
