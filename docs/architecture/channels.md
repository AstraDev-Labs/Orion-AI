...
Channel implementations use Python's `threading` module rather than asyncio. This is a deliberate choice: Orion's core inference path is synchronous, and daemon threads are simpler to compose with synchronous code than coroutines.
...
1. Create a new file in `src/orion/channels/`.
...
```python
from orion.channels._stubs import BaseChannel, ChannelMessage, ChannelStatus
...
- [API Reference: Channels](../api-reference/orion/channels/index.md) -- complete class and type signatures
