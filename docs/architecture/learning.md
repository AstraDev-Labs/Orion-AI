...
!!! note "Backward compatibility"
    The canonical locations are now `orion.learning._stubs` (for `RouterPolicy` and `QueryAnalyzer`) and `orion.core.types` (for `RoutingContext`). The old `orion.intelligence._stubs` import path still works via a backward-compatibility shim, but new code should import from `orion.learning._stubs`.
...
```bash
orion ask --router learned "What is the capital of France?"
```
...
```python
from orion.learning.router import HeuristicRouter, build_routing_context
...
```python
from orion.learning.router import build_routing_context
...
```python
from orion.learning.trace_policy import TraceDrivenPolicy
from orion.traces.analyzer import TraceAnalyzer
...
```python
from orion.learning.sft_policy import SFTRouterPolicy
# or via the backward-compat alias:
from orion.learning.sft_policy import SFTPolicy
```
...
```python
from orion.learning.agent_advisor import AgentAdvisorPolicy
```
...
```python
from orion.learning.icl_updater import ICLUpdaterPolicy
```
...
```python
from orion.learning.heuristic_reward import HeuristicRewardFunction
...
over Orion's 5-primitive configuration space. It automates finding optimal
...
The optimization framework has full Rust parity via the `orion-learning`
