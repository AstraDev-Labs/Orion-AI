...
description: The five-primitive architecture behind Orion — Intelligence, Engine, Agents, Tools, and Learning
search:
  boost: 2
---
...
Orion is a research framework for studying on-device AI systems. Its architecture is organized around **five core abstractions** -- Intelligence, Engine, Agentic Logic, Memory, and Learning -- that work together through trace-driven feedback.

![Orion Architecture](../assets/Orion_Architecture.png)
...
The sandbox module (`orion.sandbox`) adds a `SandboxedAgent` wrapper that runs any `BaseAgent` inside a Docker or Podman container with mount-security enforcement, and a `ContainerRunner` that manages the container lifecycle.
...
All extensible components in Orion use a **decorator-based registry** for runtime discovery. The pattern is implemented in `RegistryBase[T]`, a generic base class that provides isolated storage per typed subclass.
...
src/orion/
    core/               Core infrastructure shared by all primitives
        ...
        config.py           OrionConfig, hardware detection, TOML loading
    ...
    cli/                CLI commands (Click-based)
        ask.py              orion ask -- query the assistant
        serve.py            orion serve -- start API server

    sdk.py              Orion class -- high-level Python SDK
