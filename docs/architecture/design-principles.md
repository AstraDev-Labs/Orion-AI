...
Orion follows a set of design principles that guide every architectural decision. These principles ensure the framework remains extensible, portable, and easy to work with.

---

## 1. Pluggable Everything

Every major component in Orion is defined as an **abstract base class** (ABC) with concrete implementations registered at runtime. This means you can swap, extend, or replace any part of the system without modifying existing code.
...
Orion is designed to work **entirely without network access**. All core functionality -- inference, memory, agents, tools, telemetry -- operates locally. Cloud APIs are optional extensions, never requirements.
...
j = Orion(engine_key="ollama")  # Local Ollama server
...
Orion **auto-detects system hardware** at startup and recommends the optimal inference engine. The `detect_hardware()` function probes:
...
This recommendation is written to `config.toml` during `orion init` and used as the default engine:

```bash
orion init --force
# Detects hardware, writes ~/.orion/config.toml with:
# [engine]
...
```bash
orion telemetry stats          # Aggregated statistics
orion telemetry export --json  # Export all records
```
...
Orion provides a **clean Python API** through the `Orion` class. There is no framework lock-in -- the SDK is a standard Python package with dataclass-based types and no required web framework.
...
- **Optional dependencies** via extras (`orion[server]`, `orion[memory-colbert]`, etc.)
...
The API server (`orion serve`) implements the **OpenAI chat completions API format**, making Orion a drop-in replacement for OpenAI in existing applications.
...
Any OpenAI client library can connect to Orion:
...
Orion requires **no external services** for core functionality. Everything needed to run the system is included or uses standard system libraries.
...
The only external requirement is a running inference engine (Ollama, vLLM, etc.), which is the model server itself -- not a dependency of Orion.

Optional features that require additional packages:

| Feature | Extra | Packages |
|---------|-------|----------|
| FAISS memory | `orion[memory-faiss]` | `faiss-cpu`, `sentence-transformers` |
| ColBERT memory | `orion[memory-colbert]` | `colbert-ai`, `torch` |
| BM25 memory | `orion[memory-bm25]` | `rank-bm25` |
| API server | `orion[server]` | `fastapi`, `uvicorn` |
| Cloud inference | `orion[inference-cloud]` | `openai`, `anthropic`, `google-genai` |
| vLLM engine | `orion[inference-vllm]` | `vllm` |
| PDF ingestion | `orion[memory-pdf]` | `pdfplumber` |
| WhatsApp Baileys | `orion[channel-whatsapp-baileys]` | Node.js 22+ |
