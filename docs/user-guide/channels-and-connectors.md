...
Orion has two types of integrations:
...
```json
{
    "display_information": { "name": "Orion" },
    "features": {
        "app_home": {
        ...
        "bot_user": { "display_name": "Orion", "always_online": true }
    },
    "oauth_config": {
        "scopes": {
            "bot": [
...
- You DM @Orion in Slack → Socket Mode receives the event in real-time
...
| Bot not visible in Slack | Click "+" next to Direct Messages → search "Orion" |
...
3. **Connect in Orion:**
   ...
   - CLI: `uv run orion connect gmail_imap`
...
Slack serves two purposes in Orion:
...
8. **(Optional) Set the app icon:**
   ...
   - Upload the [Orion icon](https://github.com/AstraDev-Labs/Orion-AI/blob/main/assets/orion-banner.jpg)
...
- CLI: `uv run orion connect slack`
...
**DM your agent:**
- In Slack, find **Orion** under Apps (or Direct Messages)
- If you don't see it: click "+" next to Direct Messages → search "Orion"
...
- **Channel visibility:** The bot can only read channels it's been added to. Invite it with `/invite @Orion` in each channel you want indexed.
...
| No messages found (data source) | The bot can only see channels it's been added to. Invite it: `/invite @Orion` in the channel |
...
3. **Connect in Orion:**
   ...
   - CLI: `uv run orion connect notion`
...
3. **Connect in Orion:**
   ...
   - CLI: `uv run orion connect granola`
...
1. **Grant Full Disk Access** to your terminal app:
   ...
   - Enable access for Terminal, iTerm, Warp, or the Orion desktop app
...
3. **Connect in Orion:**
   ...
   - CLI: `uv run orion connect outlook`
...
2. **Connect in Orion:**
   ...
   - CLI: `uv run orion connect obsidian --path /path/to/vault`
...
4. **Connect in Orion:**
   ...
   - CLI: `uv run orion connect dropbox`
...
| Issue | Solution |
|-------|----------|
| "Connected — no data synced yet" | The connector authenticated but hasn't synced. Try running `uv run orion deep-research-setup --skip-chat` to trigger a sync. |
| Data seems stale | Connectors sync on demand. Run the setup command or click "Reconnect" to re-sync. |
| Want to reset a connector | Click "Reconnect" in the Channels tab, or delete the credential file at `~/.orion/connectors/{connector}.json` |

### Where credentials are stored

All credentials are saved locally at `~/.orion/connectors/` with file permissions `0600` (owner-only read/write). No credentials are sent to any server — everything stays on your device.
