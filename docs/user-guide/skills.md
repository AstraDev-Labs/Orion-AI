...
| **Overlay** | Sidecar file at `~/.orion/learning/skills/` storing optimized descriptions and few-shot examples |
...
orion skill install hermes:apple-notes

# Bulk install a category
orion skill sync hermes --category research

# Run a skill directly
orion skill run math-solver -a expression="41 + 82"

# See skill details
orion skill info research-and-summarize
```
...
orion skill sync hermes --category research
orion skill sync hermes --category coding
orion skill sync hermes  # everything (~150 skills)
```
...
orion skill install openclaw:0xv4l3nt1n3/etherscan

# Bulk install with search filter
orion skill sync openclaw --search "web3|crypto"
```

### From Any GitHub Repo

```bash
orion skill install github:user/repo/path/to/skill --url https://github.com/user/repo
```
...
```bash
orion skill install github:Xquik-dev/hermes-tweet/skills/hermes-tweet --url https://github.com/Xquik-dev/hermes-tweet
```

### Config-Driven Auto Import

Add sources to `~/.orion/config.toml` for automatic syncing:
...
Orion can automatically mine your trace history for recurring tool sequences and surface them as candidate skills:

```bash
# Preview discovered patterns without writing
orion skill discover --dry-run --min-frequency 3

# Write discovered skills to ~/.orion/skills/discovered/
orion skill discover
```

Discovered skills land in `~/.orion/skills/discovered/` and automatically appear in `orion skill list` on the next session.
...
orion optimize skills --dry-run

# Run DSPy optimization
orion optimize skills --policy dspy --min-traces 3

# Run GEPA evolutionary optimization
orion optimize skills --policy gepa --min-traces 3

# Inspect what optimization produced
orion skill show-overlay research-and-summarize
```

Optimization results are stored as sidecar overlays at `~/.orion/learning/skills/<skill-name>/optimized.toml`. They override the skill's description and add few-shot examples to the agent's system prompt. The original skill files are never modified.
...
# Single condition
orion bench skills --condition skills_optimized_dspy
```
...
|------|--------|-------------|---------|
| **Bundled** | Ships with Orion | Implicit trust | Full access within declared capabilities |
...
```bash
orion skill install hermes:arxiv --with-scripts
```
...
skills_dir = "~/.orion/skills/"  # where skills are installed
...
overlay_dir = "~/.orion/learning/skills/"
```
...
2. **User** `~/.orion/skills/`
3. **Bundled** (shipped with Orion)

## CLI Reference

| Command | Description |
|---------|-------------|
| `orion skill list` | List installed skills |
| `orion skill info <name>` | Show detailed skill information |
| `orion skill run <name> [-a key=value]` | Execute a skill directly |
| `orion skill install <source>:<name>` | Install from Hermes, OpenClaw, or GitHub |
| `orion skill sync [<source>] [--category C]` | Bulk install + update from sources |
| `orion skill sources` | List configured skill sources |
| `orion skill update` | Pull latest from configured sources |
| `orion skill remove <name>` | Remove an installed skill |
| `orion skill search <query>` | Search the skill index |
| `orion skill discover [--dry-run]` | Mine traces for recurring tool patterns |
| `orion skill show-overlay <name>` | Inspect optimization output for a skill |
| `orion optimize skills [--policy dspy\|gepa]` | Optimize skill descriptions + few-shot examples |
| `orion bench skills [--condition C]` | Run the PinchBench skills benchmark |
