...
- **Persistence:** SQLite database file (default: `~/.orion/memory.db`)
...
```python
from orion.tools.storage.bm25 import BM25Memory
...
!!! note "Backward compatibility"
    The old `from orion.memory.bm25 import BM25Memory` still works via backward-compatibility shims, but new code should use the canonical `orion.tools.storage.*` imports.
...
[Source: docs/intro.md] Orion is a modular AI framework...
...
orion memory index ./notes/ --chunk-size 256 --chunk-overlap 32

# Search the memory store
orion memory search "machine learning"
