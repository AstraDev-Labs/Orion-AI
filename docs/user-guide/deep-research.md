...
This writes a pre-configured `~/.orion/config.toml` for the deep research agent.
...
Orion chunks the content and stores it in a local SQLite/FTS5 database. Supported formats include `.txt`, `.md`, `.pdf`, `.py`, `.json`, `.csv`, and more.
...
orion ask "What meetings did I have with Alice last month?"

# Explicitly specify the agent
orion ask --agent deep_research "Compare the approaches described in paper-a.pdf and paper-b.pdf"

# Index more documents
orion memory index ~/Downloads/reports/
orion memory index ./notes.md

# Search memory directly
orion memory search "project timeline"
orion memory search -k 20 "budget estimates"
...
The preset writes this to `~/.orion/config.toml`:
...
orion ask "Summarize all emails about the Q3 budget review"

# Cross-reference sources
orion ask "What do papers A and B agree on regarding attention mechanisms?"

# Find specific information
orion ask "What meetings did I have with Alice last month?"

# Extract structured data
orion ask "List all action items from the meeting notes in ~/Documents/meetings/"

# Research with web fallback
orion ask "Compare our internal benchmarks with the latest published results"
```
...
# Single file
orion memory index ./report.pdf

# Custom chunk size for long documents
orion memory index ./paper.pdf --chunk-size 1024 --chunk-overlap 128
```
...
```bash
curl -s https://example.com/article | orion memory index --stdin --source "example.com"
```

### Code repositories

```bash
orion memory index ./src/ --chunk-size 256
```
...
**"No results found"** -- Make sure you have indexed documents first with `orion memory index`. Check indexed content with `orion memory stats`.
