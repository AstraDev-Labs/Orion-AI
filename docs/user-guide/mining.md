...
Orion can mine the Pearl Proof-of-Useful-Work chain through local LLM
...
Orion falls back to building from the pinned Pearl source checkout. First
builds can take 30-60 minutes.

On a shared NVIDIA host, restrict the miner to idle GPUs:

```bash
uv run orion mine init --cuda-visible-devices 0
```
...
- `orion mine models` lists Pearl model support status.
- `orion mine inspect-model` checks a Pearl model artifact before GPU launch.
- `orion mine doctor` prints hardware, Docker, Pearl node, wallet, provider,
  and session checks.
- `orion mine init` writes the local mining config and resolves the image.
- `orion mine start` launches the Pearl miner container and writes the runtime
  sidecar.
- `orion mine stop` stops the provider and removes the sidecar.
- `orion mine status` reads live gateway metrics.
- `orion mine attach` writes a sidecar for a miner you launched manually.
- `orion mine logs` prints the Docker container log tail.
- `orion mine validate-model` probes the active vLLM miner and gateway before
...
Orion only lists Pearl-compatible models published by the Pearl Research
