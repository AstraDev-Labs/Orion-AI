...
# Spotify
orion connect spotify

# Strava
orion connect strava
```
...
echo '{"api_key": "YOUR_KEY", "location": "San Francisco,CA,US"}' > ~/.orion/connectors/weather.json

# GitHub notifications (token from https://github.com/settings/tokens)
echo '{"token": "ghp_YOUR_TOKEN"}' > ~/.orion/connectors/github.json

# News RSS (no auth needed — configure your feeds)
cat > ~/.orion/connectors/news_rss.json << 'EOF'
{"feeds": [
...
```bash
CARTESIA_API_KEY="sk_car_..." orion digest --fresh
```
...
```bash
orion digest --fresh          # Generate a new digest now
orion digest                  # Show today's cached digest
orion digest --text-only      # Print text without audio
orion digest --history        # Show past digests
orion digest --schedule "0 6 * * *"   # Set daily schedule
orion digest --schedule off   # Disable schedule
orion digest --schedule       # Show current schedule
```

## Saying "Good morning"

When chatting with Orion (via CLI, desktop, or browser), saying "Good morning" or "morning digest" automatically triggers the digest — no need to use the `digest` command explicitly.
...
The `persona` field loads a prompt file from `configs/orion/prompts/personas/{name}.md`. The default `orion` persona delivers briefings with dry British wit, prioritizes urgent items, and interprets health data as trends rather than raw numbers.

To create a custom persona, add a new `.md` file in the personas directory.

### News Feeds

Add any RSS or Atom feed to `~/.orion/connectors/news_rss.json`:
...
**"No digest for today"** — Run `orion digest --fresh` to generate one.

**Empty sections** — Check connector status with `orion connect --list`. Ensure tokens haven't expired (Google/Spotify tokens expire after 1 hour and are auto-refreshed on next use).
