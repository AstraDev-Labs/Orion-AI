...
Orion provides a command-line interface through the `orion` command. Built on [Click](https://click.palletsprojects.com/), it offers subcommands for querying models, managing memory, running benchmarks, and serving an OpenAI-compatible API.

## Global Options

```bash
orion --version   # Print the Orion version
orion --help      # Show top-level help with all subcommands
```

## `orion init`

Detect local hardware (CPU, GPU, RAM) and generate a configuration file at `~/.orion/config.toml`.

```bash
orion init           # Interactive — refuses to overwrite existing config
orion init --force   # Overwrite existing config without prompting
```
...
```bash
orion ask "Explain quantum computing"
```

**Agent mode** routes the query through an agent that can use tools and manage multi-turn interactions:

```bash
orion ask --agent orchestrator "What is 2+2?"
orion ask --agent orchestrator --tools calculator,think "Calculate sqrt(144) + 3^2"
orion ask --agent simple "Hello"
```
...
# Specify a model
orion ask -m qwen3:8b "Summarize this concept"

# Use the orchestrator agent with tools
orion ask --agent orchestrator --tools calculator "What is 15% of 340?"

# Get JSON output
orion ask --json "Hello"

# Disable memory context injection
orion ask --no-context "Tell me about Python"

# Set maximum token generation
orion ask --max-tokens 2048 "Write a detailed essay about AI"
```
...
Access Pearl's native node, wallet, and RPC tools from the Orion CLI.

```bash
orion pearl doctor
orion pearl node -- <pearld args>
orion pearl wallet -- <oyster args>
orion pearl ctl -- <prlctl args>
orion pearl address
```

All Pearl wrapper commands use the `orion pearl <command>` shape. The
pass-through commands map to Pearl's native binaries:

| Orion command | Pearl binary | Use |
|--------------------|--------------|-----|
| `orion pearl doctor` | n/a | Check whether `pearld`, `oyster`, and `prlctl` are discoverable |
| `orion pearl node` | `pearld` | Run the Pearl full node |
| `orion pearl wallet` | `oyster` | Run the Oyster wallet daemon |
| `orion pearl ctl` | `prlctl` | Query Pearl node or wallet RPC |
| `orion pearl address` | `prlctl --wallet getnewaddress` | Generate a wallet address from Oyster |
...
### `orion memory index <path>`

Index documents from a file or directory into the memory store.

```bash
orion memory index ./docs/
orion memory index ./notes.md
orion memory index ./data/ --chunk-size 256 --chunk-overlap 32
orion memory index ./docs/ --backend sqlite
```
...
### `orion memory search <query>`

Search the memory store for relevant document chunks.

```bash
orion memory search "machine learning basics"
orion memory search -k 10 "neural networks"
orion memory search --backend faiss "embeddings"
```
...
```bash
orion memory stats
orion memory stats --backend sqlite
```
...
```bash
orion telemetry stats
orion telemetry stats -n 5    # Show top 5 models
```
...
### `orion telemetry export`

Export raw telemetry records in JSON or CSV format.

```bash
orion telemetry export                          # JSON to stdout
orion telemetry export --format csv             # CSV to stdout
orion telemetry export --format json -o data.json  # JSON to file
orion telemetry export -f csv -o metrics.csv    # CSV to file
```
...
### `orion telemetry clear`

Delete all telemetry records from the database.

```bash
orion telemetry clear         # Interactive confirmation
orion telemetry clear --yes   # Skip confirmation
```
...
```bash
orion bench run                               # Run all benchmarks, 10 samples
orion bench run -n 20                         # 20 samples per benchmark
orion bench run -b latency                    # Only the latency benchmark
orion bench run -b throughput -n 50 --json    # Throughput, 50 samples, JSON output
orion bench run -o results.jsonl              # Write JSONL results to file
orion bench run -m qwen3:8b -e ollama         # Specific model and engine
```
...
```bash
orion channel send slack "Hello from Orion!"
orion channel send discord "Build complete"
```
...
```bash
orion serve                                 # Default host/port from config
orion serve --port 8000                     # Custom port
orion serve --host 0.0.0.0 --port 9000      # Bind to all interfaces
orion serve --model qwen3:8b                # Specify default model
orion serve --agent orchestrator            # Route requests through an agent
```
...
`orion` subcommand for it. Construct a `SpecSearchOrchestrator`
directly from `orion.learning.spec_search.orchestrator` and call
`.run(trigger)` with a trigger from
`orion.learning.spec_search.triggers`. See
