...
README. Then write Orion' mining config:
...
```bash
orion mine doctor        # capability matrix
orion mine start         # launch gateway + miner-loop subprocesses
orion mine status        # check sidecar + gateway metrics
orion mine logs -n 120   # print recent logs
orion mine stop          # stop mining subprocesses
```
