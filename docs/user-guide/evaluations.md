...
The Orion evaluation framework (`orion-evals`) measures model **correctness and accuracy** on academic datasets. It is a separate package from the main Orion library and is designed specifically for research workflows where you need reproducible, dataset-driven quality assessments.

!!! info "Evals vs. Benchmarks"
    Orion has two distinct measurement systems that complement each other:

    | System | Package | Measures | Entry Point |
    |--------|---------|----------|-------------|
    | **Evaluations** | `orion-evals` | Correctness on academic datasets (accuracy, pass rate) | `orion-eval` |
    | **Benchmarks** | `orion` | Engine performance (latency, throughput) | `orion bench` |
...
The evaluation framework is a standalone package in the `evals/` directory. Install it alongside Orion:

```bash
uv sync --extra eval
```

This installs the `orion-eval` CLI entry point and all required dependencies (`datasets`, `huggingface-hub`, `tqdm`, `rich`).
...
These benchmarks evaluate models on practical tasks that mirror real Orion use cases.
...
```bash
uv run python -m orion.evals --config src/orion/evals/configs/use_case_v2_cloud.toml
```
...
```bash
uv run python -m orion.evals --config src/orion/evals/configs/use_case_v2_local.toml
```
...
|---------|-----|-------------|
| **orion-direct** | `orion-direct` | Engine-level inference via `SystemBuilder`. Works for local (Ollama, vLLM, llama.cpp) and cloud models. |
| **orion-agent** | `orion-agent` | Agent-level inference with tool calling. Uses `OrionSystem.ask()` with the specified agent and tools. |

Use `orion-direct` for most evaluations. Use `orion-agent` when the benchmark requires tool use — for example, GAIA tasks that reference files that must be read with `file_read`, or arithmetic tasks that benefit from `calculator`.
...
| `--backend` | | choice | `orion-direct` | `orion-direct` or `orion-agent` |
| `--model` | `-m` | str | required* | Model identifier (e.g., `qwen3:8b`, `gpt-4o`) |
| `--engine` | `-e` | str | auto | Engine key (`ollama`, `vllm`, `cloud`, ...) |
| `--agent` | | str | `orchestrator` | Agent name for `orion-agent` backend |
...
```bash
orion-eval run-all -m qwen3:8b

# With options
orion-eval run-all -m gpt-4o -n 100 --output-dir results/gpt4o/
```
...
```bash
orion-eval summarize results/supergpqa_qwen3-8b.jsonl
```
...
```bash
orion-eval run --config src/orion/evals/configs/full-suite.toml
```
...
| `backend` | str | `"orion-direct"` | Inference backend: `orion-direct` or `orion-agent` |
...
| `agent` | str | `None` | Agent name for `orion-agent` backend (e.g., `"orchestrator"`) |
| `tools` | list[str] | `[]` | Tool names for `orion-agent` backend |
...
orion-eval run -b supergpqa -m qwen3:8b -w 8 -n 500
```
...
- [Agents](agents.md) — Configure the `OrchestratorAgent` used by `orion-agent` backend
- [Tools](tools.md) — Available tools for agent-backed evaluations
- [Python SDK](python-sdk.md) — Programmatic access to Orion inference and agents
