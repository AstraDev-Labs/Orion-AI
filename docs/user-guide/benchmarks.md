...
Orion ships with two benchmarks:
...
from orion.bench._stubs import BenchmarkResult
...
from orion.bench.latency import LatencyBenchmark
from orion.bench.throughput import ThroughputBenchmark
...
orion bench run -n 50

# Run only the latency benchmark
orion bench run -b latency

# Run only the throughput benchmark with 20 samples
orion bench run -b throughput -n 20

# Specify model and engine
orion bench run -m qwen3:8b -e ollama

# Output JSON summary to stdout
orion bench run --json

# Write JSONL results to a file
orion bench run -o results.jsonl

# Combine options
orion bench run -b latency -n 100 -m qwen3:8b --json -o latency.jsonl
```
...
```python
import time
from orion.bench._stubs import BaseBenchmark, BenchmarkResult
...
```bash
orion bench run -b context_length
```
