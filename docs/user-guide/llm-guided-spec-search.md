...
```bash
python examples/orion/spec_search_quickstart.py
```
...
`orion.learning.spec_search.composite_reward.score_batch`.
...
`configs/orion/examples/spec-search-quickstart.toml`. Copy it to
`~/.orion/config.toml` (or set `OPENORION_CONFIG` to it) and the
...
| `trace_store` | `MagicMock` | `orion.traces.store.TraceStore(home / "traces.db")` |
| `student_runner` | `MagicMock` | `orion.learning.spec_search.student_runner.VLLMStudentRunner(host=..., model=...)` |
| `judge` | `MagicMock` | `orion.evals.core.scorer.LLMJudgeScorer(...)` (or a deterministic scorer if your benchmark provides one) |
| `session_store` | `MagicMock` | `orion.learning.spec_search.storage.session_store.SessionStore(home / "learning" / "sessions.db")` |
| `checkpoint_store` | `MagicMock` | `orion.learning.spec_search.checkpoint.store.CheckpointStore(home / "learning" / "checkpoints")` |
...
1. Create `src/orion/evals/datasets/<corpus>.py` implementing
   `DatasetProvider` (`adp.py` is a small reference). The provider's
   `load(max_samples, seed, split)` must respect `split` via
   `apply_split` from `orion.evals.core.splits`.
2. Register: `@DatasetRegistry.register("<corpus>")`.
3. Feed it to the proposer via the trace store:

```python
from orion.evals.datasets.adp import ADPDataset
from orion.learning.spec_search.external_adapter import (
...
store = TraceStore("~/.orion/traces.db")
...
`src/orion/evals/backends/orion_agent.py` previously hardcoded
`builder.telemetry(telemetry).traces(True).build()`, ignoring the
`telemetry` parameter. This silently caused every agent-backend
evaluation to write to `~/.orion/traces.db` regardless of caller
...
- `examples/orion/spec_search_quickstart.py` — runnable end-to-end demo.
- `configs/orion/examples/spec-search-quickstart.toml` — prebuilt config.
- `src/orion/learning/spec_search/orchestrator.py` — `SpecSearchOrchestrator` (single session).
- `src/orion/learning/spec_search/multi_session.py` — `SpecSearchLoop` (Algorithm 1).
- `src/orion/learning/spec_search/composite_reward.py` — paper Eq. 1.
- `src/orion/learning/spec_search/gate/benchmark_gate.py` — `GateOK` predicate.
- `src/orion/learning/spec_search/external_adapter.py` — corpus → trace adapter.
