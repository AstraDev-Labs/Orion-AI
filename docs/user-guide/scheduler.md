...
!!! note "Optional component"
    The scheduler is a standalone module (`orion.scheduler`). It is not wired into the default `Orion` / `OrionSystem` startup. You enable it explicitly via `SystemBuilder` or by starting the CLI daemon with `orion scheduler start`.
...
The `orion scheduler` subcommand group manages tasks and the daemon from the terminal.
...
# Active tasks only
orion scheduler list --status active

# Paused tasks
orion scheduler list --status paused
```
...
orion scheduler pause a3f9b12c4d8e

# Resume -- next_run is recomputed from the current time
orion scheduler resume a3f9b12c4d8e
```
...
orion scheduler cancel a3f9b12c4d8e
```
...
orion scheduler logs a3f9b12c4d8e
```
...
# Set up storage
store = SchedulerStore(db_path="~/.orion/scheduler.db")  # (1)!

# Wire in a OrionSystem for task execution
...
4. Starts a daemon thread named `"orion-scheduler"`. Daemon threads exit automatically when the main process exits.
...
Scheduler settings live in the `[scheduler]` section of `~/.orion/config.toml`.

```toml title="~/.orion/config.toml"
[scheduler]
enabled = false
db_path = "~/.orion/scheduler.db"
poll_interval = 60
...
| `db_path`        | `str`  | `~/.orion/scheduler.db`     | SQLite database path                       |
