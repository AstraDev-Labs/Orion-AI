...
This writes a pre-configured `~/.orion/config.toml` for the operative agent with scheduling support.
...
orion scheduler status <task-id>

# Pause / resume / delete a task
orion scheduler pause <task-id>
orion scheduler resume <task-id>
orion scheduler delete <task-id>

# Run a task immediately (outside its schedule)
orion scheduler run <task-id>

# Stop the scheduler daemon
orion scheduler stop
```
...
- **All data stays local**: State is stored in `~/.orion/` using the configured memory backend. Nothing leaves your machine.

## Troubleshooting

**"Scheduler not running"** -- Start the scheduler daemon with `orion scheduler start`. It must be running for scheduled tasks to execute.

**Task doesn't run on time** -- Check that Ollama is running (`ollama serve`). The scheduler triggers the agent, but the agent needs an inference engine. Verify the schedule with `orion scheduler status <task-id>`.
...
**Memory grows too large** -- Periodically review with `orion memory stats`. Clear old entries with `orion memory clear --before 2026-01-01` if needed.
