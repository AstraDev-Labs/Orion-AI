...
Orion includes a security layer that scans prompts and model outputs for secrets, personally identifiable information (PII), and sensitive file paths. The system is designed to be composable: scanners run as a pipeline, and the `GuardrailsEngine` wrapper drops in front of any inference backend without changing how the rest of your code works.
...
=== "Warn mode (default)"
    ...
    from orion.security.types import RedactionMode
...
from orion.security.guardrails import GuardrailsEngine
from orion.security.scanner import SecretScanner
from orion.security.types import RedactionMode
...
from orion.security.scanner import PIIScanner
...
from orion.security.file_policy import is_sensitive_file, filter_sensitive_paths
...
audit = AuditLogger(db_path="~/.orion/audit.db", bus=bus)
...
import time
from orion.security.audit import AuditLogger
from orion.security.types import SecurityEvent, SecurityEventType
...
audit = AuditLogger(db_path="~/.orion/audit.db")
...
Security settings live in the `[security]` section of `~/.orion/config.toml`.
...
| `audit_log_path` | `str` | `~/.orion/audit.db` | Path to the SQLite audit log |
...
from orion.security.types import ScanFinding, ScanResult, ThreatLevel
...
- [API Reference: Security](../api-reference/orion/security/index.md) — full class and function signatures
