...
This writes a pre-configured `~/.orion/config.toml` for the code assistant.
...
```bash
orion ask "Write a Python script that reads a CSV file and prints the top 5 rows"
```
...
orion ask "Write a Python script that parses JSON from stdin"

# Read and explain existing code
orion ask "Read main.py and explain the architecture"

# Fix a bug
orion ask "Find and fix the bug in test_utils.py"

# Run tests
orion ask "Run the test suite and summarize any failures"

# Explicitly specify agent and tools
orion ask --agent orchestrator --tools code_interpreter "Calculate the first 20 Fibonacci numbers"
...
```bash
# Write a new script
orion ask "Write a Python script that converts YAML to JSON"

# Explain existing code
orion ask "Read src/orion/core/events.py and explain the EventBus pattern"

# Debug a failing test
orion ask "Run pytest tests/test_memory.py -v and fix any failures"

# Refactor code
orion ask "Read utils.py and refactor the parse_config function to use dataclasses"

# Generate tests
orion ask "Read src/orion/tools/calculator.py and write unit tests for it"

# Shell tasks
orion ask "Find all Python files larger than 100KB in this repo"
```
...
- The agent asks for confirmation before executing potentially destructive commands when running in interactive mode (`orion chat`).
- For stronger isolation, use the sandboxed agent: `orion ask --agent sandboxed --tools code_interpreter "..."`, which runs inside a Docker/Podman container.
...
**Shell command fails** -- The `shell_exec` tool runs commands relative to where you launched `orion`. Use `cd /path && command` in your prompt if needed, or run `orion` from the project directory.
