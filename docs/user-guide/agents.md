...
Agents are the agentic logic layer of Orion. They determine how a query is processed -- whether it goes directly to a model, through a tool-calling loop, via ReAct reasoning, CodeAct code execution, recursive decomposition, or an external agent runtime. All agents implement the `BaseAgent` ABC and are registered via the `AgentRegistry`.
...
from orion.agents._stubs import AgentContext, AgentResult
...
!!! note "Backward compatibility"
    The registry alias `"react"` maps to `NativeReActAgent`. The old import `from orion.agents.react import ReActAgent` also still works.
...
```bash
# Simple agent
orion ask --agent simple "What is the capital of France?"

# Orchestrator with tools
orion ask --agent orchestrator --tools calculator,think "What is sqrt(256)?"

# NativeReActAgent
orion ask --agent native_react --tools calculator "What is 2+2?"

# ReAct alias (same as native_react)
orion ask --agent react --tools calculator,think "Solve step by step: 15% of 340"

# NativeOpenHandsAgent
orion ask --agent native_openhands --tools calculator,web_search "Summarize example.com"

# RLMAgent
orion ask --agent rlm "Summarize this long document"

# OpenHands SDK agent
orion ask --agent openhands "Fix the bug in test_utils.py"
```
...
!!! warning "Requirements"
    Requires Node.js 22+ on `PATH` and an `ANTHROPIC_API_KEY` environment variable (or pass `api_key=` directly). The bundled runner is auto-installed to `~/.orion/claude_code_runner/` on first use via `npm install`.

**How it works:**

1. On first call, copies the bundled `claude_code_runner/` to `~/.orion/claude_code_runner/` and runs `npm install --production` if `node_modules` is missing.
...
4. The Python side parses the output between `---OPENORION_OUTPUT_START---` and `---OPENORION_OUTPUT_END---` markers, extracting content, tool results, and metadata.
...
**When to use:** For software engineering tasks where the Claude Agent SDK's built-in tools (code editing, bash execution, file operations) provide capabilities beyond what Orion tool-calling agents support.

```python
from orion.agents.claude_code import ClaudeCodeAgent
...
```bash
# Via CLI
orion ask --agent claude_code "Refactor the tests to use pytest fixtures"
```

!!! info "accepts_tools = False"
    `ClaudeCodeAgent` does not accept Orion tools via `--tools`. Tool access for the Claude agent is configured separately via the `allowed_tools` constructor parameter, which passes tool names understood by the Claude Agent SDK itself.
...
```python
from orion.agents.operative import OperativeAgent
...
```bash
# Via CLI
orion ask --agent operative "Check system status"
```
...
```python
from orion.agents.monitor_operative import MonitorOperativeAgent
...
```bash
# Via CLI
orion ask --agent monitor_operative "Analyze the security audit findings"
```
...
```python
from orion.sandbox import ContainerRunner, SandboxedAgent
from orion.agents.simple import SimpleAgent

runner = ContainerRunner(
    image="orion-sandbox:latest",
    timeout=60,
    mount_allowlist_path="/etc/orion/mount_allowlist.json",
)
...
| Parameter              | Type   | Default                      | Description                                    |
|------------------------|--------|------------------------------|------------------------------------------------|
| `image`                | `str`  | `"orion-sandbox:latest"`| Docker image to run                            |
