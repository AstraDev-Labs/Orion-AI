...
prove Orion wiring; this validates that a real H100/H200 host can mine
through Pearl and serve inference through Orion.
...
- Docker container `orion-pearl-miner` starts.
- `~/.orion/runtime/mining.json` is written.
...
```bash
docker ps --filter name=orion-pearl-miner
cat ~/.orion/runtime/mining.json
uv run orion mine logs --tail 200
...
## Verify Orion Inference Uses Mining Endpoint

Run:

```bash
uv run orion mine doctor
uv run orion ask "Say hello in one sentence."
```
...
- If metric names differ, attach `/tmp/pearl-gateway-metrics.txt` to the PR and
  update `src/orion/mining/_metrics.py`.
...
```bash
uv run orion mine stop
docker ps --filter name=orion-pearl-miner
test ! -e ~/.orion/runtime/mining.json
```
...
- Container stops.
- Sidecar is removed.
- `orion ask` no longer routes through `vllm-pearl-mining` unless another
...
- Orion inference succeeds through `vllm-pearl-mining`.
...
```bash
uv run orion mine doctor
uv run orion mine status || true
uv run orion mine logs --tail 300 || true
docker inspect orion-pearl-miner || true
