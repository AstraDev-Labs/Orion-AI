...
through Pearl's vLLM miner and Orion.

Orion can point `vllm-pearl` at a model id, but a raw Hugging Face model is
...
Orion only supports Pearl models published by the `pearl-ai` Hugging Face
organization. Private staging artifacts and Orion-specific conversion
...
`orion mine start`, vLLM `/v1/models`, Orion inference routing, Pearl
gateway template refresh, and `orion mine validate-model`.

`pearl-ai/Gemma-4-31B-it-pearl` and
`pearl-ai/Llama-3.1-8B-Instruct-pearl` are listed because they are public Pearl
org artifacts. They remain `planned` in Orion until we have clean
...
2. Convert the target model.
   ...
   Orion includes an experimental local converter for this work:
   ...
   its output as a staging artifact until `orion mine inspect-model` and
   `orion mine validate-model` pass on H100/H200 hardware.
...
3. Validate the Pearl vLLM plugin path.
   - Run `orion mine inspect-model --model <pearl-model-id>
...
4. Validate chain integration.
   ...
   - `orion mine status` parses those metrics.

5. Promote the model in Orion.
...
```text
src/orion/mining/_models.py
```

`orion mine models` renders that registry. Planned models are visible to users
...
- `orion mine inspect-model --model <pearl-model-id> --allow-planned`
- `orion mine init --model <pearl-model-id>`
- `orion mine start`
- `curl http://127.0.0.1:8000/v1/models`
- `orion ask "Say hello in one sentence."`
- `orion mine status`
- `orion mine validate-model --model <pearl-model-id> --allow-planned --prompt
...
Attach the JSON artifact from `orion mine validate-model --output` to the issue.
