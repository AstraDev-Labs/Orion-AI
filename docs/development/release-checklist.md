...
2. **`orion` (no args)** drops into a chat session within 2 s.
...
6. **`orion doctor`** exits 0 once all bg work completes; shows the Background tasks table.
7. **Re-run `curl … | bash`** on the same machine. It completes ≤ 30 s, says `[ok] step already done` for every step.
8. **`orion-uninstall`** removes `~/.orion/` and `~/.local/bin/orion*`. Verify with `ls`.
...
- Delete `~/.orion/config.toml` — verify bare `orion` re-runs init.
- Delete `~/.orion/.venv` — verify re-running curl heals it.
