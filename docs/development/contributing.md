...
```bash
uv run orion --version   # Should print 0.1.0
uv run orion --help      # Show all subcommands
```
...
Orion uses [pytest](https://docs.pytest.org/) with approximately 1,000+
...
```bash
uv run pytest tests/ --cov=orion --cov-report=html
```
...
Orion uses [Ruff](https://docs.astral.sh/ruff/) for linting, configured
in `pyproject.toml`:
...
The source code is organized under `src/orion/`:
...
src/orion/
    ...
    core/                       # Shared infrastructure
        config.py               # OrionConfig, hardware detection, TOML loader
    ...
    cli/                        # Click CLI commands
        ...
        ask.py                  # orion ask
        init_cmd.py             # orion init
        model.py                # orion model list/info
        memory_cmd.py           # orion memory index/search/stats
        telemetry_cmd.py        # orion telemetry stats/export/clear
        bench_cmd.py            # orion bench run
        serve.py                # orion serve
```
...
try:
    import orion.memory.faiss_backend  # noqa: F401
except ImportError:
    pass
```
