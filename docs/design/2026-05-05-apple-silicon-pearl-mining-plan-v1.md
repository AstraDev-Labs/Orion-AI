...
**Architecture:** New provider `CpuPearlProvider` in `src/orion/mining/cpu_pearl.py`, registered as `"cpu-pearl"` via `MinerRegistry`. The provider's `start()` launches two subprocesses: (1) Pearl's `pearl-gateway` Python service (talks to user's `pearld`), and (2) a small Python miner-loop that polls gateway for `getMiningInfo`, calls `pearl_mining.mine()`, and submits via `submitPlainProof`. Reuses Spec A's `MiningProvider` ABC, `MinerRegistry`, sidecar at `~/.orion/runtime/mining.json`, telemetry adapter, and CLI surface (`orion mine init|start|stop|status|doctor`) — all unchanged.
...
src/orion/mining/__init__.py  # Soft-import cpu_pearl
src/orion/mining/_constants.py  # Add PEARL_PINNED_REF and helper constants
```
...
**Files:**
- Modify: `src/orion/mining/_constants.py:1-N` (created in Spec A)
...
Run: `cat src/orion/mining/_constants.py`
...
Append to `src/orion/mining/_constants.py`:
...
CPU_PEARL_LOCAL_CLONE_DIR = "~/.orion/cache/pearl"
...
Run: `uv run ruff check src/orion/mining/_constants.py`
Expected: no errors.

- [ ] **Step 4: Commit**

```bash
git add src/orion/mining/_constants.py
...
**Files:**
- Create: `src/orion/mining/_install.py`
...
```python
"""Tests for orion.mining._install."""
...
def test_pearl_packages_available_returns_false_when_pearl_mining_missing():
    from orion.mining import _install
...
def test_install_hint_is_actionable():
    ...
    from orion.mining._install import install_hint
...
Expected: FAIL — `ImportError: cannot import name '_install' from 'orion.mining'`
...
```bash
git add src/orion/mining/_install.py tests/mining/test_install.py
...
**Files:**
- Modify: `src/orion/mining/_install.py`
...
Run: `uv run pytest tests/mining/test_install.py -v`
Expected: FAIL — `AttributeError: module 'orion.mining._install' has no attribute 'build_from_pin'`

- [ ] **Step 3: Implement `build_from_pin`**

Append to `src/orion/mining/_install.py`:
...
Run: `uv run ruff check src/orion/mining/_install.py tests/mining/test_install.py`
Expected: no errors.
...
**Files:**
- Create: `src/orion/mining/_miner_loop_main.py`
...
```python
"""Tests for orion.mining._miner_loop_main."""
from __future__ import annotations
...
def test_decode_mining_info_returns_header_and_target(fake_mining_info_response):
    from orion.mining._miner_loop_main import _decode_mining_info
...
def test_encode_plain_proof_round_trips():
    ...
    from orion.mining._miner_loop_main import _encode_plain_proof
...
def test_jsonrpc_envelope_shape():
    ...
    from orion.mining._miner_loop_main import _make_request
...
Run with:
    python -m orion.mining._miner_loop_main \
...
logger = logging.getLogger("orion.mining.miner_loop")
...
Run: `uv run ruff check src/orion/mining/_miner_loop_main.py tests/mining/test_miner_loop.py`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add src/orion/mining/_miner_loop_main.py tests/mining/test_miner_loop.py
...
**Files:**
- Create: `src/orion/mining/_pearl_subprocess.py`
...
```python
"""Tests for orion.mining._pearl_subprocess."""
from __future__ import annotations
...
def test_launcher_start_spawns_two_processes(fake_popen, tmp_path):
    from orion.mining._pearl_subprocess import PearlSubprocessLauncher
...
- ``orion.mining._miner_loop_main`` (this repo), which polls the gateway
...
Run: `uv run ruff check src/orion/mining/_pearl_subprocess.py tests/mining/test_pearl_subprocess.py`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add src/orion/mining/_pearl_subprocess.py tests/mining/test_pearl_subprocess.py
...
**Files:**
- Modify: `src/orion/mining/_pearl_subprocess.py`
...
Run: `find ~/.orion/cache/pearl/miner/pearl-gateway -name config.py -path '*pearl_gateway*' 2>/dev/null || find / -name config.py -path '*pearl_gateway*' 2>/dev/null | head -3`

Expected: `pearl-gateway/src/pearl_gateway/config.py` (path depends on where Pearl was cloned). If the file isn't on disk yet, run `python -m orion.mining._install` (added in Task 4) or clone Pearl manually first.
...
Open `src/orion/mining/_pearl_subprocess.py` and replace each guessed env name with the actual one. Keep the dict structure; only the keys change.
...
```bash
git add src/orion/mining/_pearl_subprocess.py
...
**Files:**
- Create: `src/orion/mining/cpu_pearl.py`
...
```python
"""Tests for orion.mining.cpu_pearl.CpuPearlProvider."""
from __future__ import annotations
...
def darwin_apple_hw():
    ...
    from orion.mining._stubs import HardwareInfo, GpuInfo
...
def windows_hw():
    ...
    from orion.mining._stubs import HardwareInfo

    return HardwareInfo(platform="win32", cpu_arch="x86_64", gpu=None)


def test_detect_supported_on_apple_silicon(darwin_apple_hw):
    from orion.mining.cpu_pearl import CpuPearlProvider

    with patch(
        "orion.mining._install.pearl_packages_available", return_value=True
    ):
...
def test_detect_unsupported_when_pearl_not_installed(darwin_apple_hw):
    ...
    with patch(
        "orion.mining._install.pearl_packages_available", return_value=False
    ):
...
Run: `uv run ruff check src/orion/mining/cpu_pearl.py tests/mining/test_cpu_pearl.py`
Expected: no errors.

- [ ] **Step 6: Commit**

```bash
git add src/orion/mining/cpu_pearl.py tests/mining/test_cpu_pearl.py
...
**Files:**
- Modify: `src/orion/mining/cpu_pearl.py`
...
def test_start_writes_sidecar_and_returns_running(darwin_apple_hw, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "orion.mining.cpu_pearl._sidecar_path",
    ...
    with patch(
        "orion.mining.cpu_pearl.PearlSubprocessLauncher",
...
Replace `src/orion/mining/cpu_pearl.py` with the full implementation:
...
def _sidecar_path() -> Path:
    return Path(os.path.expanduser("~/.orion/runtime/mining.json"))


def _log_dir() -> Path:
    return Path(os.path.expanduser("~/.orion/logs/mining"))
...
Run: `uv run ruff check src/orion/mining/cpu_pearl.py`
Expected: no errors.
...
**Files:**
- Modify: `src/orion/mining/__init__.py`
...
Run: `cat src/orion/mining/__init__.py`
...
Append to `src/orion/mining/__init__.py` (or modify if Spec A already defined `ensure_registered`):

```python
def _register_cpu_pearl() -> None:
    """Register CpuPearlProvider in MinerRegistry. Idempotent."""
    from orion.core.registry import MinerRegistry  # type: ignore
...
# Call at import time so the registry is populated as soon as orion.mining
...
```bash
git add src/orion/mining/__init__.py
...
Orion can mine the [Pearl](https://github.com/pearl-research-labs/pearl) chain
...
```bash
orion mine init    # OJ will detect the missing wheels and offer to build
...
|---|---|---|
| `mine doctor` says `Pearl Python packages not installed` | Wheels not built yet | Run `orion mine init` |
| `pearl-gateway` log shows `connection refused` to `http://localhost:44107` | `pearld` not running | Start `pearld` per Pearl's README |
| `mine status` shows `last_error: gateway metrics unreachable` | `pearl-gateway` crashed | Check `~/.orion/logs/mining/pearl-gateway.log` |
...
For anything not on this list, capture `~/.orion/logs/mining/` and open
an issue at https://github.com/AstraDev-Labs/Orion/issues.
...
def test_provider_runs_end_to_end_on_this_host(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "orion.mining.cpu_pearl._log_dir",
...
- `MiningProvider` ABC at `src/orion/mining/_stubs.py`
- `MinerRegistry` class at `src/orion/core/registry.py`
- `MiningConfig`, `MiningCapabilities`, `MiningStats`, `HardwareInfo` dataclasses
- Sidecar location convention `~/.orion/runtime/mining.json`
