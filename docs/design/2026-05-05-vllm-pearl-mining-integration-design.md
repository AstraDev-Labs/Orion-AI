...
### In scope (v1)
- New `orion.mining` subsystem with `MiningProvider` ABC, `MinerRegistry`, `MiningCapabilities` / `MiningConfig` / `MiningStats` dataclasses
- `vllm-pearl` provider implementation: orchestrate Pearl's published `vllm-miner` Docker container
- `[mining]` TOML section in OJ config; `MiningConfig` field in `OrionConfig`
- New CLI namespace: `orion mine init|start|stop|status|doctor|attach|logs`
- Runtime sidecar at `~/.orion/runtime/mining.json` for engine ↔ mining handoff
...
| Engine attachment | Runtime sidecar JSON at `~/.orion/runtime/mining.json` | Existing vLLM engine class stays untouched. Sidecar is the single source of truth tying mining lifecycle to engine resolution. Inspectable via `cat`. |
...
src/orion/cli/
    mine_cmd.py          # orion mine init|start|stop|status|doctor|attach|logs
...
`MinerRegistry` added to `src/orion/core/registry.py` as a peer to `EngineRegistry`, `AgentRegistry`, etc. `tests/conftest.py`'s autouse `_clean_registries` fixture is updated to include `MinerRegistry.clear()`.
...
[mining.extra]
docker_image_tag         = "orion/pearl-miner:<pinned-ref>"
...
`~/.orion/runtime/mining.json` (created on `mine start`, removed on `mine stop`):
...
1. `orion mine start` → `MinerRegistry.get("vllm-pearl").start(config)`.
...
4. `orion ask` and the SDK route to that endpoint transparently. The user's normal inference is the mining work.
...
Power users running their own Pearl container skip `orion mine start` and write the sidecar themselves via `orion mine attach --vllm-endpoint=... --gateway-url=...`. Decouples lifecycle from wiring.
...
| Command | Purpose |
|---|---|
| `orion mine init` | Interactive: hardware/Docker checks, prompt for wallet + pearld credentials, write `[mining]`, pull/build image. Does NOT start mining. Pre-checks `>=200 GB` free disk. |
| `orion mine start` | Launch container via the registered provider, write sidecar, print endpoint info. Idempotent if running. |
| `orion mine stop` | Stop container, remove sidecar. Idempotent if not running. |
| `orion mine status` | Read sidecar + query gateway `:8339/metrics`. Print `MiningStats`. |
| `orion mine doctor` | Capability matrix; every check ✓/✗ with reason. Works in any state. |
| `orion mine attach` | Manual mode: write sidecar without launching. |
| `orion mine logs [-f]` | Tail container logs through Docker SDK. |
...
Image
  orion/pearl-miner:<ref>   present (built 2026-04-30)   ✓
Pearl node
...
- Existing `com.orion.gateway` daemon is unchanged.
...
- launchd/systemd installation surface (`orion daemon install`) untouched.

### 6.5 Concurrency

POSIX `flock` on `~/.orion/runtime/mining.lock` prevents racing `mine start` invocations.

### 6.6 `orion ask` UX hint

When `[mining]` is configured but no sidecar exists, `cli/hints.py` emits one line: `"mining configured but not running — start it with \`orion mine start\`"`. One-line UX nudge, no new infrastructure.
...
| **Build-from-pin** | OJ git-clones Pearl at a pinned ref into `~/.orion/cache/pearl/`, then `docker buildx build` | v1 default (Pearl publishes nothing today) |
...
- **`auto_remove=False`** so a crashed container stays around for `orion mine logs` post-mortem.
...
- `orion mine init` triggers `ensure_image()`, streams build/pull output through the CLI with a clear time estimate (`"Building Pearl miner image — first run takes ~45 min on a fast machine"`).
- `orion mine doctor` reports `image: present (tag, age, sha)` or `image: missing (run mine init)`.
- `orion mine prune` (v1.x) cleans old `orion/pearl-miner:*` tags. Manual `docker image rm` works in v1.
...
- **v1: on-demand only.** `orion mine status` makes one HTTP GET per call (~10 ms). No background polling.
...
- `orion telemetry stats --mining` (v1.x) joins to the latest `MiningStats` snapshot and reports `tokens / share`, `joules / share`, `est. PRL / kWh`.
...
**1. `submit_target` parsed into a tagged union; only one variant works.** `SoloTarget` accepted at runtime in v1; `PoolTarget` raises `NotImplementedError("pool support is v2 — track orion#XYZ")`. Reachable only by users who edit their config to opt in.
...
| Concurrent `mine start` | POSIX `flock` on `~/.orion/runtime/mining.lock`; second invocation errors clearly | clear message |
