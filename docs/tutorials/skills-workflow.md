...
!!! note "Before you begin"
    This tutorial assumes Orion is installed with Ollama running and a model available (e.g., `qwen3.5:9b`). If you have not completed setup yet, start with the [Quick Start guide](../getting-started/quickstart.md).

## Step 1: Install Skills from Hermes Agent

Orion can import skills from the [Hermes Agent](https://github.com/NousResearch/hermes-agent) skill library maintained by NousResearch. Let's install a few useful ones.

```bash
# Install individual skills
orion skill install hermes:arxiv
orion skill install hermes:github-pr-workflow

# Or bulk install an entire category
orion skill sync hermes --category research
```

The first install clones the Hermes repo to `~/.orion/skill-cache/hermes/` (one-time, ~5s). Subsequent installs reuse the cache.
...
```bash
cat ~/.orion/skills/hermes/arxiv/SKILL.md | head -40
```

The `.source` file records provenance:

```bash
cat ~/.orion/skills/hermes/arxiv/.source
```
...
```bash
orion ask "Use the math-solver skill to compute 17 * 23" \
  --engine ollama --model qwen3.5:9b
```
...
```bash
mkdir -p ~/.orion/skills/my-reviewer
```

Write a SKILL.md:

```bash
cat > ~/.orion/skills/my-reviewer/SKILL.md << 'EOF'
---
name: my-reviewer
...
```bash
orion ask "Use the my-reviewer skill to review this function: def login(user, pwd): return db.query(f'SELECT * FROM users WHERE name={user} AND pass={pwd}')" \
  --engine ollama --model qwen3.5:9b
```
...
orion ask "Use math-solver to compute 100 / 7"
orion ask "Use code-explainer to explain: lambda x: x**2"
orion ask "Use my-reviewer to review: def add(a,b): return a+b"
orion ask "Use math-solver to compute 2**10"
orion ask "Use code-explainer to explain: [x for x in range(10) if x % 2 == 0]"
```

Each query produces a trace in `~/.orion/traces.db` with skill metadata tags (`skill`, `skill_source`, `skill_kind`).
...
orion skill discover --dry-run --min-frequency 2

# Write discovered patterns as skill manifests
orion skill discover --min-frequency 2
```
...
This produces overlay files at `~/.orion/learning/skills/<skill-name>/optimized.toml` with improved descriptions and few-shot examples extracted from your best traces.

Inspect what was produced:

```bash
orion skill show-overlay math-solver
orion skill show-overlay code-explainer
```
...
For a hands-off experience, add this to `~/.orion/config.toml`:
...
|---------|-------------|
| **Installing skills** | `orion skill install hermes:arxiv` — imported from public sources |
| **Using skills** | `orion ask "Use the code-explainer skill..."` — agent invokes skills as tools |
...
| **Discovering patterns** | `orion skill discover` — mined traces for recurring tool sequences |
| **Optimizing skills** | `orion optimize skills --policy dspy` — improved descriptions + few-shot examples |
| **Benchmarking** | `orion bench skills` — measured the impact across 4 conditions |
