---
title: Installation
description: Get Orion running — browser app, desktop app, CLI, or Python SDK
search:
  boost: 3
---
...
Orion runs entirely on your hardware. Choose the interface that fits your workflow.
...
The desktop app is a native window for the Orion chat UI. All inference and backend
...
!!! warning "macOS: \"app is damaged\""
    ...
    ```bash
    xattr -cr /Applications/Orion.app
    ```
...
The command-line interface is the fastest way to interact with Orion
...
For programmatic access, the `Orion` class provides a high-level sync API.
...
Orion uses optional extras to keep the base installation lightweight.
...
!!! note "Ollama, vLLM, and llama.cpp are HTTP-based"
    These engines have no additional Python dependencies — Orion communicates over HTTP. You still need the engine software running on your machine.
...
| Extra | Install Command | Description |
|-------|----------------|-------------|
| `server` | `uv sync --extra server` | OpenAI-compatible API server (`orion serve`) |
...
Orion requires at least one inference backend. Choose the one that matches your hardware.
