---
title: Configuration
description: Complete reference for Orion configuration
---

# Configuration

Orion uses a TOML configuration file to control engine selection, model identity, memory backends, agent behavior, and more. This page is the complete reference for every configuration option, organized by primitive.
...
Orion creates the `~/.orion/` directory and populates it with a default config when you run `orion init`.
...
3. Writes `~/.orion/config.toml` with sensible defaults
...
|-------|------|---------|-------------|
| `default` | string | Auto-detected | Default engine backend. One of: `ollama`, `vllm`, `llamacpp`, `sglang`, `cloud`. Set automatically by `orion init` based on hardware detection. |
...
!!! tip "Engine fallback"
    If the configured default engine is unreachable, Orion automatically probes all registered engines and falls back to any healthy one.
...
When both `default_model` and `fallback_model` are empty, Orion uses the configured router policy (see `[learning]`) to select a model from those available on the active engine.
...
```bash
orion ask --router heuristic "Hello"
```
...
| `db_path` | string | `~/.orion/memory.db` | Path to the SQLite memory database. Used by the `sqlite` backend. |
...
Controls the OpenAI-compatible API server started by `orion serve`.
...
```bash
orion serve --host 127.0.0.1 --port 9000 --model qwen3:8b --agent simple
```
...
[traces]
enabled = false
db_path = "~/.orion/traces.db"
```

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled` | bool | `false` | Whether to record traces for each agent interaction. |
| `db_path` | string | `~/.orion/traces.db` | Path to the SQLite trace database. |
...
| `skills_dir` | string | `~/.orion/skills/` | Directory where skills are installed. |
...
| `overlay_dir` | string | `~/.orion/learning/skills/` | Where optimized skill overlays are stored. |
...
When you run `orion init`, Orion probes your system to detect available hardware. The detection runs in this order:
...
If you have an existing `~/.orion/config.toml` from a previous version, here is what changed and how to update it.
...
You can configure Orion entirely from Python without a TOML file:
...
```python
j = Orion(config_path="/path/to/my-config.toml")
```
...
Orion respects the following environment variables:
