...
ORION is a sophisticated AI assistant designed for multimodal interaction. It combines Google's Gemini 2.5 Native Audio with computer vision, gesture control, and 3D CAD generation in a Electron desktop application.
...
ORION's "Minority Report" interface uses your webcam to detect hand gestures:
...
graph TB
    ...
    subgraph Backend ["Backend (Python 3.11 + FastAPI)"]
        ...
        ORION[orion.py<br/>Gemini Live API]
...
```bash
# 1. Clone and enter
git clone https://github.com/AstraDev-Labs/orion.git && cd orion_project

# 2. Create Python environment (Python 3.11)
conda create -n orion_project python=3.11 -y && conda activate orion_project
...
# 5. Run!
conda activate orion_project && npm run dev
```
...
2. Type this command and hit Enter:
   ```bash
   git clone https://github.com/AstraDev-Labs/orion.git
   ```
3. This creates a folder named `orion_project`.

**Step 5: Open in VS Code**
1. Open VS Code.
2. Go to **File > Open Folder**.
3. Select the `orion_project` folder you just downloaded.
...
```bash
conda create -n orion_project python=3.11
conda activate orion_project
...
To use the secure voice features, ORION needs to know what you look like.

1. Take a clear photo of your face (or use an existing one).
2. Rename the file to `reference.jpg`.
3. Drag and drop this file into the `orion_project/backend` folder.
...
ORION can slice STL files and send them directly to your 3D printer.
...
ORION uses **OrcaSlicer** (recommended) or PrusaSlicer to generate G-code.
1. Download and install [OrcaSlicer](https://github.com/SoftFever/OrcaSlicer).
2. Run it once to ensure profiles are created.
3. ORION automatically detects the installation path.

**Step 2: Connect Printer**
1. Ensure your printer and computer are on the **same Wi-Fi network**.
2. Open the **Printer Window** in ORION (Cube icon).
3. ORION automatically scans for printers using mDNS.
...
ORION uses Google's Gemini API for voice and intelligence. You need a free API key.

1. Go to [Google AI Studio](https://aistudio.google.com/app/apikey).
2. Sign in with your Google account.
3. Click **"Create API Key"** and copy the generated key.
4. Create a file named `.env` in the `orion_project` folder (same level as `README.md`).
...
You have two options to run the app. Ensure your `orion_project` environment is active!

### Option 1: The "Easy" Way (Single Terminal)
The app is smart enough to start the backend for you.
1. Open your terminal in the `orion_project` folder.
2. Activate your environment: `conda activate orion_project`
3. Run:
   ```bash
   npm run dev
   ```
...
1. **Voice Check**: Say "Hello ORION". She should respond.
...
### 🖨️ Printing & Slicing
- **Auto-Discovery**: ORION automatically finds printers on your network.
- **Slicing**: Click "Slice & Print" on any generated 3D model.
- **Profiles**: ORION intelligently selects the correct OrcaSlicer profile based on your printer's name (e.g., "Creality K1").
...
**Solution**:
1. Make sure your `.env` file is in the root `orion_project` folder (not inside `backend/`).
...
**Solution**:
This is a server-side issue from the Gemini API. Simply reconnect by clicking the connect button or saying "Hello ORION" again. If it persists, check your internet connection or try again later.
...
│   ├── orion.py                  # Gemini Live API integration
...
<p align="center">
  <strong>Built with 🤖 by Tharun</strong><br>
