...
MODEL="${OPENORION_MODEL:-qwen3:0.6b}"
...
uv run maturin develop -m rust/crates/orion-python/Cargo.toml --quiet 2>/dev/null \
...
uv run orion serve --port 8000 &>/dev/null &
CLEANUP_PIDS+=($!)
sleep 3
...
echo ""
echo -e "${GREEN}${BOLD}  Orion is running!${NC}"
echo ""
