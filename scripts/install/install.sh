#!/usr/bin/env bash
# install.sh — Orion curl-pipe-bash installer.
#
# Usage:
...
Orion installs to $HOME/.orion, not /usr/local. Re-run as your
...
SCRIPTS_DIR="$OPENORION_HOME/.scripts"
STATE_FILE="$STATE_DIR/install-state.json"

mkdir -p "$OPENORION_HOME" "$STATE_DIR" "$SCRIPTS_DIR"

# ---- WSL detection ----
WSL=0
if [[ "${OPENORION_FORCE_WSL:-0}" == "1" ]]; then
    WSL=1
...
# Orion team can see where users drop off during install.
...
clone_repo() {
    ...
    git clone --depth 1 "$OPENORION_REPO_URL" "$SRC_DIR"
}
...
write_config() {
    "$VENV_DIR/bin/orion" _bootstrap --write-config \
...
install_symlinks() {
    ...
    ln -sf "$SCRIPTS_DIR/orion-wrapper.sh" "$HOME/.local/bin/orion"
    ln -sf "$SCRIPTS_DIR/orion-uninstall.sh" "$HOME/.local/bin/orion-uninstall"
}
...
ensure_path() {
    ...
    if grep -q "Orion" "$rc" 2>/dev/null; then
        return 0
    fi
    {
        echo ''
...
from orion.core.config import detect_hardware, recommend_model
hw = detect_hardware()
...
# ---- run ----
echo "Orion installer"
echo "  install dir: $OPENORION_HOME"
echo "  WSL2:        $WSL"
echo
...
step clone_repo         "Clone Orion repo" clone_repo
step copy_scripts       "Copy install scripts"  copy_scripts
step create_venv        "Create venv"           create_venv
step editable_install   "Install Orion"    editable_install
