...
//!
//! Rust translation of `src/orion/core/config.py`.
...
/// Default config directory: `~/.orion/`
pub fn default_config_dir() -> PathBuf {
    dirs_home().join(".orion")
}

/// Default config file path: `~/.orion/config.toml`
...
fn default_sandbox_image() -> String { "orion-sandbox:latest".into() }
...
fn default_operators_dir() -> String { "~/.orion/operators".into() }
...
pub fn load_config(path: Option<&Path>) -> Result<OrionConfig, ConfigError> {
    ...
    let mut cfg = if config_path.exists() {
        ...
        let mut cfg: OrionConfig = toml::from_str(&content)?;
        ...
        let mut cfg = OrionConfig::default();
        cfg.engine.default = recommended_engine;
        cfg
    };
...
mode = "block"
"#;
        let cfg: OrionConfig = toml::from_str(toml_str).unwrap();
