...
use orion_core::error::{EngineError, OrionError};
use orion_core::{GenerateResult, Message, ToolCall, Usage};
...
impl InferenceEngine for VLLMEngine {
    ...
    fn generate(
        ...
        max_tokens: i64,
        extra: Option<&Value>,
    ) -> Result<GenerateResult, OrionError> {
        ...
        let resp = self
            ...
            .send()
            .map_err(|e| {
                OrionError::Engine(EngineError::Connection(format!(
        ...
        if !resp.status().is_success() {
            ...
            return Err(OrionError::Engine(EngineError::Http(format!(
                "vLLM returned {}: {}",
                status, body
            ))));
        }
        ...
        let data: Value = resp.json().map_err(|e| {
            OrionError::Engine(EngineError::Deserialization(e.to_string()))
        })?;
    ...
    async fn stream(
        ...
        let async_client = reqwest::Client::builder()
            ...
            .build()
            .map_err(|e| {
                OrionError::Engine(EngineError::Connection(e.to_string()))
            })?;
        ...
        let token_stream = byte_stream.filter_map(|chunk_result| async {
            match chunk_result {
                ...
                Err(e) => Some(Err(OrionError::Engine(EngineError::Streaming(
                    e.to_string(),
                )))),
            }
        });
    ...
    fn list_models(&self) -> Result<Vec<String>, OrionError> {
        let resp = self
            .client
            ...
            .send()
            .map_err(|_| {
                OrionError::Engine(EngineError::Connection(
