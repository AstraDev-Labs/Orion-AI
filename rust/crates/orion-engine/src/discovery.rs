...
pub fn discover_engines(config: &OrionConfig) -> Vec<EngineInfo> {
...
pub fn get_engine_static(
...
) -> Result<crate::engine_enum::Engine, OrionError> {
    ...
    match key.as_str() {
        ...
        other => Err(OrionError::Engine(
            orion_core::error::EngineError::ModelNotFound(format!(
                "Unknown engine: {}",
                other
            )),
        )),
