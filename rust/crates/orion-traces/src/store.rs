...
use orion_core::{OrionError, Trace};
...
impl TraceStore {
    pub fn new(db_path: &Path) -> Result<Self, OrionError> {
        if let Some(parent) = db_path.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                OrionError::Io(std::io::Error::other(e))
            })?;
        }

        let conn = Connection::open(db_path).map_err(|e| {
            OrionError::Io(std::io::Error::other(e.to_string()))
        })?;
    ...
    pub fn in_memory() -> Result<Self, OrionError> {
        Self::new(Path::new(":memory:"))
    }

    pub fn save(&self, trace: &Trace) -> Result<(), OrionError> {
    ...
    pub fn get(&self, trace_id: &str) -> Result<Option<Trace>, OrionError> {
        let conn = self.conn.lock();
        let mut stmt = conn
            .prepare(
            ...
            .map_err(|e| {
                OrionError::Io(std::io::Error::other(
                    e.to_string(),
                ))
            })?;
    ...
    pub fn count(&self) -> Result<usize, OrionError> {
        let conn = self.conn.lock();
        let count: i64 = conn
