...
use orion_core::{AgentContext, AgentResult, OrionError, ToolResult};
use orion_tools::executor::ToolExecutor;
use regex::Regex;
    ...
    async fn run(
            ...
            .map(|ctx| {
                ctx.conversation
                    ...
                    .filter_map(|m| match m.role {
                        orion_core::Role::User => {
                            Some(RigMessage::user(&m.content))
                        }
                        orion_core::Role::Assistant => {
        ...
        for turn in 1..=self.max_turns {
            let response = self
                ...
                .await
                .map_err(|e| {
                    OrionError::Agent(orion_core::error::AgentError::Execution(
                        e.to_string(),
                    ))
                })?;
...
mod tests {
    ...
    type OpenHandsAgent = NativeOpenHandsAgent<RigModelAdapter<orion_engine::Engine>>;
