//! PyO3 bridge — exposes ~50 Rust classes to Python via `orion_rust`.
...
fn load_config(path: Option<&str>) -> PyResult<core::PyConfig> {
    ...
    let config = orion_core::load_config(p)
...
fn detect_hardware() -> PyResult<String> {
    let hw = orion_core::hardware::detect_hardware();
    Ok(serde_json::to_string(&hw).unwrap_or_default())
}

#[pyfunction]
fn check_ssrf(url: &str) -> Option<String> {
    orion_security::check_ssrf(url)
}

#[pyfunction]
fn is_sensitive_file(path: &str) -> bool {
    orion_security::is_sensitive_file(std::path::Path::new(path))
}

#[pyfunction]
fn register_builtin_models() {
    orion_core::model_catalog::register_builtin_models();
}

#[pyfunction]
fn classify_query(query: &str) -> &'static str {
    orion_learning::classify_query(query)
}

#[pymodule]
fn orion_rust(m: &Bound<'_, PyModule>) -> PyResult<()> {
    // --- Core types ---
