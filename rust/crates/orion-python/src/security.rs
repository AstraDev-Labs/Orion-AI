...
pub struct PySecretScanner {
    inner: orion_security::SecretScanner,
}
...
impl PySecretScanner {
    ...
    fn new() -> Self {
        Self {
            inner: orion_security::SecretScanner::new(),
        }
    }
...
pub struct PyPIIScanner {
    inner: orion_security::PIIScanner,
}
...
impl PyPIIScanner {
    ...
    fn new() -> Self {
        Self {
            inner: orion_security::PIIScanner::new(),
        }
    }
...
pub struct PyGuardrailsEngine {
    inner: orion_security::GuardrailsEngine<orion_engine::Engine>,
}
...
impl PyGuardrailsEngine {
    ...
    fn new(
        ...
        let redaction_mode = match mode {
            "redact" => orion_security::RedactionMode::Redact,
            "block" => orion_security::RedactionMode::Block,
            _ => orion_security::RedactionMode::Warn,
        };
        Ok(Self {
            inner: orion_security::GuardrailsEngine::new(
                engine,
                redaction_mode,
                scan_input,
                scan_output,
    ...
    fn engine_id(&self) -> &str {
        use orion_engine::InferenceEngine;
...
pub struct PyAuditLogger {
    inner: parking_lot::Mutex<orion_security::AuditLogger>,
}
...
impl PyAuditLogger {
    ...
    fn new(path: Option<&str>) -> PyResult<Self> {
        ...
        let inner = orion_security::AuditLogger::new(&db_path)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string()))?;
        Ok(Self {
...
pub struct PyCapabilityPolicy {
    inner: orion_security::CapabilityPolicy,
}
...
impl PyCapabilityPolicy {
    ...
    fn new(default_deny: bool) -> Self {
        Self {
            inner: orion_security::CapabilityPolicy::new(default_deny),
        }
    }
...
pub struct PyInjectionScanner {
    inner: orion_security::InjectionScanner,
}
...
impl PyInjectionScanner {
    ...
    fn new() -> Self {
        Self {
            inner: orion_security::InjectionScanner::new(),
        }
    }
...
pub struct PyRateLimiter {
    inner: orion_security::RateLimiter,
}
...
impl PyRateLimiter {
    ...
    fn new(requests_per_minute: u32, burst_size: u32) -> Self {
        Self {
            inner: orion_security::RateLimiter::new(
                orion_security::RateLimitConfig {
                    requests_per_minute,
                    burst_size,
                    enabled: true,
                },
...
impl PyTaintSet {
    ...
    fn new() -> Self {
        Self {
            inner: orion_security::TaintSet::new(),
        }
    }

    fn add(&mut self, label: &str) {
        let taint_label = match label {
            "pii" => orion_security::TaintLabel::Pii,
            "secret" => orion_security::TaintLabel::Secret,
            "user_private" => orion_security::TaintLabel::UserPrivate,
            "external" => orion_security::TaintLabel::External,
            _ => orion_security::TaintLabel::External,
        };
        // TaintSet is immutable-style; union with a single-label set.
        self.inner = self.inner.union(
            &orion_security::TaintSet::from_labels(&[taint_label]),
        );
    }
