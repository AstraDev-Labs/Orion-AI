...
pub struct PyWorkflowGraph {
    inner: orion_workflow::WorkflowGraph,
}
...
impl PyWorkflowGraph {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: orion_workflow::WorkflowGraph::new(name),
        }
    }
    ...
    fn add_node(
        ...
        let nt = match node_type {
            "tool" => orion_workflow::NodeType::Tool,
            "condition" => orion_workflow::NodeType::Condition,
            "parallel" => orion_workflow::NodeType::Parallel,
            "loop" => orion_workflow::NodeType::Loop,
            "transform" => orion_workflow::NodeType::Transform,
            _ => orion_workflow::NodeType::Agent,
        };
        let node = orion_workflow::WorkflowNode {
            id: id.to_string(),
            node_type: nt,
    ...
    fn add_edge(&mut self, source: &str, target: &str) -> PyResult<()> {
        let edge = orion_workflow::WorkflowEdge {
...
pub struct PyWorkflowBuilder {
    inner: Option<orion_workflow::WorkflowBuilder>,
}
...
impl PyWorkflowBuilder {
    ...
    fn new(name: &str) -> Self {
        Self {
            inner: Some(orion_workflow::WorkflowBuilder::new(name)),
        }
    }
