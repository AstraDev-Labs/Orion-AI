"""Tests for intent-based agent routing in OrionSystem."""
...
class TestDetectAgentIntent:
    ...
    @pytest.fixture()
    def system(self):
        """Create a minimal OrionSystem instance for testing _detect_agent_intent."""
        from orion.system import OrionSystem

        mock_engine = MagicMock()
        mock_engine.engine_name = "mock"
        sys = OrionSystem.__new__(OrionSystem)
    ...
    def test_good_morning_triggers_digest(self, system):
        with patch("orion.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            assert system._detect_agent_intent("Good morning!") == "morning_digest"

    def test_good_morning_orion_triggers_digest(self, system):
        with patch("orion.core.registry.AgentRegistry") as reg:
            reg.contains.return_value = True
            result = system._detect_agent_intent("Good morning Orion")
