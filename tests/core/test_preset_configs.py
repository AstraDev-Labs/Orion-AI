...
Presets are installed via `orion init --preset <name>`, which copies
`configs/orion/examples/<name>.toml` to `~/.orion/config.toml`.
...
def test_preset_loads(preset_path: Path) -> None:
    ...
    # slots `orion init` expects to be populated for a working first run.
