...
from orion.cli import _version_check
from orion.cli._version_check import (
    _check_disabled,
    _config_disabled,
...
def _clean_env(monkeypatch, tmp_path):
    for v in ("OPENORION_NO_UPDATE_CHECK", "CI", "OPENORION_CONFIG"):
        monkeypatch.delenv(v, raising=False)
    # Point config + cache at empty tmp paths so tests don't see the
    # developer's real ~/.orion state.
    monkeypatch.setenv("OPENORION_CONFIG", str(tmp_path / "no-config.toml"))
...
class TestCheckDisabled:
    ...
    def test_orion_no_update_check_disables(self, monkeypatch, value):
        monkeypatch.setenv("OPENORION_NO_UPDATE_CHECK", value)
...
class TestConfigDisabled:
    ...
    def test_auto_update_false_disables(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setenv("OPENORION_CONFIG", str(cfg))
    ...
    def test_orion_config_env_override(self, monkeypatch, tmp_path):
        """OPENORION_CONFIG should redirect the lookup, matching core.config."""
...
class TestCheckForUpdates:
    @patch("orion.cli._version_check._do_check")
    ...
    @patch(
        "orion.cli._version_check._do_check",
