"""Tests for the ``orion eval`` CLI commands."""
...
class TestEvalCLI:
    ...
    def test_eval_group_exists(self):
        """``orion eval --help`` shows run/compare/report/list subcommands."""
    ...
    def test_eval_list_benchmarks(self):
        """``orion eval list`` exits 0 and outputs benchmark names."""
        ...
        assert "orion-direct" in result.output
        assert "orion-agent" in result.output

    def test_eval_run_missing_args(self):
        """``orion eval run`` without required args fails gracefully."""
