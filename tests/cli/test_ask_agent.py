"""Tests for ``orion ask --agent`` CLI integration."""
...
from orion.core.types import ToolCall, ToolResult
from orion.tools._stubs import BaseTool, ToolSpec

_ask_mod = importlib.import_module("orion.cli.ask")
...
def mock_setup():
    ...
    with (
        ...
        mock_cfg.return_value = OrionConfig()
...
class TestAskAgentOption:
    ...
    def test_no_agent_flag_falls_back_to_config_default_agent(
        ...
        The default ``OrionConfig`` sets ``default_agent = "simple"``, so
        ``orion ask "..."`` should route through SimpleAgent rather than
...
class TestPersonaFilesReachModel:
    ...
    def test_soul_md_content_reaches_engine_in_simple_agent(
        ...
        soul.write_text("PERSONA_SENTINEL_zh_orion", encoding="utf-8")
        ...
        assert "PERSONA_SENTINEL_zh_orion" in joined
