...
class TestClassifyMentionDispatch:
    ...
    def test_valid_labels_pass_through(self, llm_label, expected):
        ...
        assert _classify_mention("some tweet", orion=j) == expected
    ...
    def test_defaults_to_question_if_model_returns_other(self):
        ...
        assert _classify_mention("hahaha", orion=j) == "QUESTION"
...
# =========================================================================
# 1c. Prompt-injection detector (unit, mocked Orion)
# =========================================================================
...
class TestPromptBuilders:
    ...
    def test_bug_prompt_contains_github_url(self):
        ...
        assert "api.github.com/repos/open-orion/Orion/issues" in prompt
...
class TestMentionPolling:
    ...
    def test_poll_dispatches_to_handler(self):
        ...
        assert msg.content == "@OrionAI how do I install?"
...
class TestEnvVarExpansion:
    ...
    def test_github_token_expanded(self):
        ...
        with (
            ...
            patch(
                "orion._rust_bridge.get_rust_module",
                return_value=mock_rust,
            ),
            patch("orion.tools.http_request.check_ssrf", return_value=None),
            patch(
                "orion.tools.http_request.httpx.request",
...
# =========================================================================
# 5. Full reactive e2e flow (mock Orion + TwitterChannel)
# =========================================================================
...
class TestFullE2EFlow:
    ...
    def _make_mock_orion(self, responses=None):
        """Create a mock Orion instance that returns canned responses."""
        j = MagicMock()
        if responses:
    ...
    def test_question_flow(self):
        ...
        """
        j = self._make_mock_orion(["check the docs at open-orion.github.io"])
        tweet = DEMO_TWEETS[0]
    ...
    def test_bug_report_flow(self):
        ...
        j = self._make_mock_orion(["opened an issue for this"])
        tweet = DEMO_TWEETS[1]
    ...
    def test_praise_flow(self):
        ...
        j = self._make_mock_orion(["thanks, glad you like it!"])
        tweet = DEMO_TWEETS[3]
        mention_type = "PRAISE"
    ...
    def test_spam_is_ignored(self):
        """Spam mentions should be skipped — no Orion.ask call."""
        j = self._make_mock_orion()
    ...
    def test_all_demo_tweets_processed(self):
        ...
        TestClassifyMentionDispatch against a mocked orion. This test
...
class TestGitHubIssueCreation:
    ...
    def test_create_bug_issue(self):
        ...
        mock_resp.text = json.dumps({
            "number": 42,
            "html_url": "https://github.com/open-orion/Orion/issues/42",
        })
        mock_resp.headers = {"content-type": "application/json"}

        with (
            patch.dict(os.environ, {"GITHUB_TOKEN": "ghp_testtoken123"}),
            patch("orion._rust_bridge.get_rust_module", return_value=mock_rust),
            patch("orion.tools.http_request.check_ssrf", return_value=None),
            patch(
                "orion.tools.http_request.httpx.request",
                return_value=mock_resp,
