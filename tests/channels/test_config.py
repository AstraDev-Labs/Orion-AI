...
from orion.core.config import ChannelConfig, OrionConfig, load_config
...
class TestChannelConfigInOrionConfig:
    def test_channel_config_in_orion_config(self) -> None:
        cfg = OrionConfig()
        assert hasattr(cfg, "channel")
        assert isinstance(cfg.channel, ChannelConfig)

    def test_orion_config_channel_defaults(self) -> None:
        cfg = OrionConfig()
