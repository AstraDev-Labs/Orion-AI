...
setup() {
    ...
    export OPENORION_HOME="$FAKE_HOME/.orion"
    ...
    # uv stub needs to fake creating a venv with a orion binary.
...
case "\$1" in
    venv)
        ...
        if [[ -n "\$venv_path" ]]; then
            ...
            cat > "\$venv_path/bin/orion" <<'EOJ'
#!/usr/bin/env bash
# fake orion for tests
echo "fake orion: \$@"
exit 0
EOJ
            chmod +x "\$venv_path/bin/orion"
...
@test "creates ~/.orion directory tree" {
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    [ -d "$OPENORION_HOME/src" ]
    [ -d "$OPENORION_HOME/.state" ]
    [ -d "$OPENORION_HOME/.scripts" ]
}

@test "writes install-state.json on success" {
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    [ -f "$OPENORION_HOME/.state/install-state.json" ]
}

@test "calls git clone for the Orion repo" {
    run bash "$SCRIPT" --no-bg-orchestrator
    grep -q "clone" "$GIT_STUB_LOG"
}

@test "creates orion symlink in ~/.local/bin" {
    run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    [ -L "$FAKE_HOME/.local/bin/orion" ] || [ -f "$FAKE_HOME/.local/bin/orion" ]
}
...
@test "detects WSL2 and writes platform note to install-state" {
    OPENORION_FORCE_WSL=1 run bash "$SCRIPT" --no-bg-orchestrator
    [ "$status" -eq 0 ]
    grep -q "wsl" "$OPENORION_HOME/.state/install-state.json"
}
