"""Tests for orion.evals.comparison.table_gen."""
...
class TestRenderBooktabs:
    def test_emits_valid_tabular(self) -> None:
        from orion.evals.comparison.table_gen import _render_booktabs
        ...
        assert "hermes" in fragment and "orion" in fragment
...
class TestT1Builder:
    def test_t1_builds_from_synthetic_results(self) -> None:
        ...
        assert "orion-distilled" in fragment.lower() or "OJ-distilled" in fragment
...
class TestT3to7Builders:
    ...
    def test_builder_emits_tabular(self, builder_name: str) -> None:
        import orion.evals.comparison.table_gen as m

        builder = getattr(m, builder_name)
        rows = []
        for fwk in ["hermes", "orion"]:
...
class TestTableBuilderRegistry:
    def test_registry_has_all_seven(self) -> None:
        from orion.evals.comparison.table_gen import _TABLE_BUILDERS
...
class TestTableGenCLI:
    def test_cli_writes_fragment_and_preview(self, tmp_path: Path) -> None:
        ...
        from orion.evals.comparison.table_gen import main
