"""Tests for orion.evals.backends.external.hermes_agent.HermesBackend."""
...
from orion.evals.backends.external._subprocess_runner import SubprocessResult
    ...
    return ThirdPartyConfig(
        entries={
            "hermes": ThirdPartyEntry(
                ...
                runner_script=(
                    "src/orion/evals/backends/external/_runners/hermes_runner.py"
                ),
                python_executable="",
            )
        }
...
class TestHermesBackend:
    def test_generate_full_builds_correct_subprocess_command(
        ...
        with (
            patch(
                "orion.evals.backends.external.hermes_agent.load_third_party_config",
                return_value=cfg,
            ),
            patch(
                "orion.evals.backends.external.hermes_agent.verify_commit_pin",
            ),
            patch(
                "orion.evals.backends.external.hermes_agent.run_one_shot",
            ) as mock_run,
        ):
