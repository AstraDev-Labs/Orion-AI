"""Verify OrionDirectBackend.generate_full returns the spec §6.2 extended fields."""
...
class TestOrionDirectExtendedFields:
    def test_generate_full_includes_framework_and_commit(self) -> None:
        ...
        # engine); set required attrs directly. OrionDirectBackend.generate_full
        # calls ``self._system.engine.generate(messages, ...)`` so we mock the
        # whole ``_system`` chain.
        backend = OrionDirectBackend.__new__(OrionDirectBackend)
