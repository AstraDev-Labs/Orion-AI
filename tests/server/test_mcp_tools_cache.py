...
def test_returns_tools_from_mcp_server(mock_load_config: MagicMock):
    ...
    with (
        patch("orion.mcp.transport.StreamableHTTPTransport"),
        patch("orion.mcp.client.MCPClient"),
        patch("orion.tools.mcp_adapter.MCPToolProvider") as MockProvider,
    ):
