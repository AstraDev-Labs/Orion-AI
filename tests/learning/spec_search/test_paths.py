"""Tests for orion.learning.spec_search.storage.paths module."""
...
class TestResolveSpecSearchRoot:
    ...
    def test_default_is_under_home(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from orion.learning.spec_search.storage import paths

        monkeypatch.delenv("OPENORION_HOME", raising=False)
        result = paths.resolve_spec_search_root()
        assert result == Path.home() / ".orion" / "learning"

    def test_respects_orion_home_env_var(
        self,
        tmp_path: Path,
        ...
        monkeypatch.setenv("OPENORION_HOME", str(custom))
        result = paths.resolve_spec_search_root()
    ...
    def test_returns_absolute_path(
        ...
        monkeypatch.setenv("OPENORION_HOME", str(tmp_path / "rel"))
        result = paths.resolve_spec_search_root()
    ...
    def test_rejects_path_inside_source_tree(
        ...
        # Find the Orion source root by walking up from the paths module.
        source_root = paths._find_source_root()
        assert source_root is not None  # We must be running inside the repo.

        # Force OPENORION_HOME to point inside the source tree.
        monkeypatch.setenv("OPENORION_HOME", str(source_root / "junk_dir"))
...
class TestEnsureSpecSearchDirs:
    ...
    def test_creates_subdirs(
        ...
        monkeypatch.setenv("OPENORION_HOME", str(tmp_path / "oj"))
