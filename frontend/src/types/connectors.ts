...
export const SOURCE_CATALOG: ConnectorMeta[] = [
  ...
  // ── Communication ──────────────────────────────────────────────────
  {
    ...
    steps: [
      {
        label: 'Make sure 2-Step Verification is enabled, then generate a 16-character App Password (Mail / Other / "Orion"). Paste it below \u2014 spaces are fine, and use the app password, not your regular Gmail password.',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to api.slack.com/apps and click "Create New App" → choose "From scratch". Name it "Orion" and pick your workspace',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to notion.so/profile/integrations → click "+ New integration". Name it "Orion", select your workspace, and click Submit',
      ...
      {
        label: 'To share ALL your pages at once: open any top-level page → click "..." (top right) → "Connections" → "Add connections" → search "Orion" → click it. This shares the page and all its sub-pages. Repeat for each top-level page, or share your entire workspace by doing this on every root page',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Click the "+" button at the bottom of the list. Navigate to Applications → Utilities → select "Terminal.app" (or iTerm2/Warp if you use those). If you\'re using the desktop app, also add "Orion.app" from Applications',
      },
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart Orion). iMessage data will be detected automatically — no credentials needed',
      },
    ],
  },
  // ── Documents ──────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        label: 'Paste the full path below. Orion will index all .md files in the vault',
      },
    ],
    inputFields: [
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to Google Cloud Console → create a new project (or select an existing one). Give it any name (e.g. "Orion")',
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart Orion). Apple Notes will be detected automatically — no credentials needed',
      },
    ],
  },
  {
    ...
    steps: [
      ...
      {
        label: 'Toggle the switch ON next to each app you added. Close and reopen your terminal (or restart Orion). Apple Contacts will be detected automatically — no credentials needed',
      },
    ],
  },
  {
    ...
    steps: [
      {
        label: 'Go to the Azure Portal → App Registrations → click "+ New registration". Name it "Orion", select "Accounts in this organizational directory only", and click Register',
  ...
  },
  {
    ...
    steps: [
      {
        label: 'Go to the Dropbox App Console and click "Create app". Choose "Scoped access" → "Full Dropbox" → give it a name (e.g. "Orion") → click "Create app"',
