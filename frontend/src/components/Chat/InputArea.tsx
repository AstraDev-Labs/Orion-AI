...
export function InputArea() {
        ...
        if (eventName === 'agent_turn_start') {
          ...
          try {
            ...
            const tc: ToolCallInfo = {
              id: generateId(),
              tool: data.tool,
              arguments: typeof data.arguments === 'object' ? JSON.stringify(data.arguments) : (data.arguments || ''),
              status: 'running',
            };
            toolCalls.push(tc);
            setStreamState({
          ...
          try {
            ...
            if (tc) {
              ...
              tc.result = typeof data.result === 'object' ? JSON.stringify(data.result) : data.result;
            }
            setStreamState({
              phase: 'Generating...',
              activeToolCalls: [...toolCalls],
  ...
  return (
    <div className="px-4 pb-4 pt-2 relative z-20" style={{ maxWidth: 'var(--chat-max-width)', margin: '0 auto', width: '100%' }}>
      <div className="mb-3 flex flex-col gap-1">
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-1.5 rounded-lg text-[10px] font-mono uppercase tracking-widest transition-all duration-300 cursor-pointer disabled:cursor-default disabled:opacity-50 border relative overflow-hidden backdrop-blur-md"
            style={{
              background: deepResearch ? 'rgba(168, 85, 247, 0.15)' : 'rgba(11, 17, 32, 0.6)',
              borderColor: deepResearch ? 'rgba(168, 85, 247, 0.5)' : 'rgba(168, 85, 247, 0.15)',
              color: deepResearch ? 'var(--color-accent)' : 'var(--color-text-tertiary)',
              boxShadow: deepResearch ? '0 0 15px rgba(168, 85, 247, 0.2)' : 'none',
            }}
            title={deepResearch ? 'Deep Research: active' : 'Deep Research: offline'}
          >
            <div className={`w-2 h-2 rounded-sm ${deepResearch ? 'bg-purple-400 animate-pulse glow-text-purple' : 'bg-purple-900'}`} />
            <Search size={12} className={deepResearch ? 'text-purple-400' : 'text-purple-800'} />
            Deep Research
          </button>
        </div>
        {deepResearch && corpusSync.syncing && corpusSync.itemsSynced > 0 && (
          <div className="text-[10px] leading-snug font-mono uppercase tracking-widest text-purple-400/80 mt-1 pl-1">
            [SCANNING_CORPUS: COMPILING_INDEX] Searching over{' '}
            <span key={corpusSync.itemsSynced} className="sync-bump text-purple-300 font-bold glow-text-purple">
              {corpusSync.itemsSynced.toLocaleString()}
            </span>{' '}
            objects &middot; sync active
          </div>
        )}
      </div>

      <div
        className="flex items-center gap-3 rounded-2xl px-5 py-4 transition-all duration-300 relative border border-white/10"
        style={{
          background: isFocused ? 'rgba(255, 255, 255, 0.08)' : 'rgba(255, 255, 255, 0.03)',
          backdropFilter: 'blur(24px)',
          WebkitBackdropFilter: 'blur(24px)',
          boxShadow: isFocused 
            ? '0 0 30px rgba(168, 85, 247, 0.15), inset 0 1px 1px rgba(255, 255, 255, 0.15)' 
            : '0 8px 32px 0 rgba(0, 0, 0, 0.4), inset 0 1px 1px rgba(255, 255, 255, 0.05)',
        }}
      >
        ...
        <textarea
          ...
          rows={1}
          className="flex-1 bg-transparent outline-none resize-none text-base leading-relaxed font-light placeholder:text-white/30 text-white"
        ...
        {streamState.isStreaming ? (
          <button
            ...
            className="p-2.5 rounded-lg transition-all duration-200 shrink-0 cursor-pointer hover:scale-105 active:scale-95 bg-red-950/50 border border-red-500/50 text-red-400 hover:bg-red-900/60 shadow-[0_0_15px_rgba(239,68,68,0.3)]"
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2.5 rounded-full transition-all duration-200 shrink-0 cursor-pointer disabled:opacity-30 disabled:cursor-default disabled:scale-100 hover:scale-105 active:scale-95 text-white"
              style={{
                background: input.trim() ? 'rgba(168, 85, 247, 0.8)' : 'rgba(255, 255, 255, 0.1)',
      ...
      <div className="flex items-center justify-center mt-3 text-[10px] font-light text-white/40">
        <span>
          Press <kbd className="font-medium text-white/70 bg-white/10 px-1.5 py-0.5 rounded mr-1.5">Enter</kbd> to send,{' '}
          <kbd className="font-medium text-white/70 bg-white/10 px-1.5 py-0.5 rounded mx-1.5">Shift+Enter</kbd> for a new line
        </span>
      </div>
    </div>
  );
