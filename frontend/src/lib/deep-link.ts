/**
 * deep-link.ts — Handler for orion:// deep links in the Tauri desktop app.
 *
 ...
 *   orion://research/{session_id}   → open a research session
 *   orion://connector/{connector_id} → open a connector settings panel
 */
...
/**
 * Parse an `orion://` deep link URL into a structured target.
 *
 * @param url - The raw deep link URL string (e.g. `orion://research/abc123`).
 * @returns A {@link DeepLinkTarget} if the URL is valid, or `null` if it
 *   cannot be parsed or does not use the `orion:` scheme.
 *
 * @example
 * parseDeepLink("orion://research/abc123");
 * // → { type: "research", id: "abc123" }
 *
 * parseDeepLink("orion://connector/gmail");
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== "orion:") return null;

    // The URL constructor treats "orion://research/abc123" such that
